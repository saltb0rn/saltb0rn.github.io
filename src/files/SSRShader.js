		...
		void main(){
			...
			vec3 viewNormal=getViewNormal( vUv );
				...
				vec3 viewReflectDir=reflect(viewIncidentDir,viewNormal);
                        ...
                        // 反射光线的最大搜索距离与相机视角相关, 当相机正对物体表面时, 距离应该偏小; 当表面时掠射角时 , 反射距离应该增加
			...
			for(float i=0.;i<float(MAX_STEP);i++){
				...
				if(viewReflectRayZ<=vZ){
                                // viewReflectRayZ 反射光线当前位置的视点 z 分量, 视野中的视点坐标的 z 分量小于 0
                                // viewReflectRayZ<=vZ 意味着反射光线进入物体中了
					...
					if(hit){
						vec3 vN=getViewNormal( uv );
                                                // vN 是交点处的法线, dot(viewReflectDir,vN) < 0. 保证反射光线是正面射向物体表面
						if(dot(viewReflectDir,vN)>=0.) continue;
                                                // 保证交点 vP 与 viewPosition 所属平面的距离不超过 maxDistance
