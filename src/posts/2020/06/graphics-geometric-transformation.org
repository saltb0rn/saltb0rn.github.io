...
**** 拓展思考
     ...
     经过旋转 $p$ 的坐标为: $Mp = \left(\begin{array}{c} (x_{p} - x_{q})\cos \theta - (y_{p} - y_{q})\sin \theta + x_{q} \\ (x_{p} - x_{q})\sin \theta + (y_{p} - y_{q})\cos \theta + y_{q} \end{array}\right \\ 1)$.
...
*** 关于变换的一些其它概念
    ...
    很多图形软件都不是让用户使用矩阵来对图形进行变换的,除了旋转多少度这样比较符合直觉的常见变换以外,

    还有以下方式来描述变换,实际上它们的核心和上面的矩阵基本是一样的.
