...
欧几里得空间(Euclidean space)是一套度量空间(这里的空间真的是时空里的空间),基于向量空间补充一些新定义(向量的长度,以及向量之间的一些"乘法",这些会在文章后面需要用上的时候进行补充),
...
任何通过原点的直线或者平面都是欧几里得的子线性空间.

那些没有通过原点的直线或者平面就是仿射空间的一个例子,它们也具备线性,所以通俗点理解:仿射空间相当于线性空间去掉原点.
