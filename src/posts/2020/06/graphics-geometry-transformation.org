...
所谓的几何变换就是一个坐标变换,把它看做一个函数就是: 输入几何对象里面的所有点,输出一个新的几何对象.

举个例子,假如 $p$ 点在坐标系下 $A$ 中坐标是 $\left(x, y\right)$,假设现在坐标 $B$ 是 $A$ 逆时针旋转 $\theta$ 度后得到的,

同样 $p$ 也跟着旋转到达 $p^{'}$ 这个位置, $p^{'}$ 在 $B$ 上也是 $\left(x, y\right)$,现在要求 $p^{'}$ 在 $A$ 上的坐标 $\left(x^{'}, y_{'}\right)$.

这跟坐标变换(coordinate transformation)不同,在于几何变换只是在改变点,而坐标变换只是改变坐标系,但是由于坐标与点之间的相对性来说,变换其中一个就是在以另外一个作为参照物,
