...
***** 围绕任意通过原点的轴进行旋转
      ...
      *补图*
      ...
      假设现在有 $\vec{u} = \left(x_{u}, y_{u}, z_{u}\right)$ 以及 $\vec{v} = \left(x_{v}, y_{v}, z_{v}\right)$, 叉积为 $\vec{u} \times \vec{v} = \left(\begin{array}{c}0 & -z_{u} & y_{u} \\ z_{u} & 0 & -u_{x} \\ -y_{u} & u_{x} & 0 \end{array}\right)\vec{v}$, 点积为 $\vec{u} \cdot \vec{v} = x_{u}x_{v} + y_{u}y_{v} + z_{u}z_{v}$.(要注意,这只能适用于原点为 $\left(0, 0, 0\right)$ 的坐标系).
