...
第一步是就是选择相机看到的内容,因为相机是不能看到完整场景的,所以要求出相机看到的内容,再把看到的内容投影到空间上的一个平面上,这个空间叫做裁剪空间(clip space),

这一步就是从视点坐标变成裁剪坐标,也就是从视点坐标系到裁剪坐标系(clip coordinate system),这个时候只是确定看到的内容,还没有真正投影,也没有发生真正的裁剪.

这里涉及了一个细节: 当远处的对象被近处的对象挡住,只有前面的对象会出现在图片上,这个就是隐藏面问题(hidden surface problem),解决这个问题的算法叫做深度测试(depth test);

第二步,对变换后的坐标进行裁剪(clip/frustum clip),实际上这才是确定裁剪空间.裁剪的计算是这样的,首先,裁剪用的坐标是这样的 $\left(x, y, z, w\right)$,没错就是齐次坐标,用 $w$ 坐标分别比较 $x$, $y$ 和 $z$ 坐标,如果任何一个坐标小于 $-w$ 或者大于 $w$,

那么该顶点就会被舍弃,所以一个点要出现在图片上必须满足 $-w < x,y,z < w$ 这个关系,
