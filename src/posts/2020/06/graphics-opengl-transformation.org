...
- 方法一:
  ...
  不过, 我们知道视点空间的所有坐标的 $w$ 分量是 1, 可得 $w_{e} = 1$.

  这里用 $(M_{projection}^{-1}p^{'}_{n})_{w}$ 表示获取运算结果中的 $w$ 分量, 可以得到: $w_{e} = w_{p}(M_{projection}^{-1}p^{'}_{n})_{w} = 1$.

  进而得到 $w_{p} = \frac{1}{(M_{projection}^{-1}p^{'}_{n})_{w}}$.

  $p^{'}_{e} = M_{projection}^{-1}p^{'}_{n}$

  $p_{e} = w_{p}p^{'}_{e}$

  只要先计算出 $p^{'}_{e}$, 再让 $p^{'}_{e}$ 除以自身的 $w^{'}_{e}$ 分量就可以求出 $p_{e}$ 了: $p_{e} = \frac{p^{'}_{e}}{p^{'}_{w}} = \left(\begin{array}{c} \frac{x^{'}_{e}}{w^{'}_{e}} \\ \frac{x^{'}_{e}}{w^{'}_{e}} \\ \frac{x^{'}_{e}}{w^{'}_{e}} \\ \frac{w^{'}_{e}}{w^{'}_{e}} = 1 \end{array}\right)$.
