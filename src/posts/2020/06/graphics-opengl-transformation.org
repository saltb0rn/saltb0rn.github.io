...
如果 $\vec{n}$ 是单位向量,并且 $p_{1}$ 到原点的距离为 $d$, 那么 $p_{1}$ 可以表示成 $\left(da, db, dc\right)$ ($\vec{n}$ 就是 $\vec{p_{1}}$ 的方向),整个等式变为 $ax + by + cz - d \cdot \left(a^{2} + b^{2} + c^{2}\right) = 0$,

又因为 $\vec{n}$ 是单位向量,所以 $a^{2} + b^{2} + c^{2} = 1$,所以最终等式为: $ax + by + cz - d = 0$,也有不少人写成 $ax + by + cz + d$ 的,两种都可以.

如果 $\vec{n}$ 不是单位向量,那就自己对 $\vec{n}$ 进行一般化,也就是求出它的单位向量,然后进行同样的推导.反过来说,如果 $p$ 满足这个等式就证明 $p$ 来自与平面 $P$.

这是在欧几里得坐标系下的平面方程,那在齐次坐标系下呢?其实推导结果都差不多的,假设 $p = \left(x, y, z, w\right)$, $\vec{n} = \left(a, b, c, k\right)$ 并且是单位向量, $p_{1} = \left(da, db, dc, dk\right)$,

$\vec{n} \cdot \left(\vec{p} - \vec{p_{1}}\right) = 0$

$\left(a, b, c, k\right) \cdot \left(x - da, y - db, z - dc, w - dk\right) = 0$

$ax + by + cz + kw - d \cdot \left(a^{2} + b^{2} + c^{2} + k^{2}\right) = 0$
...
=OpenGL= 法线向量只能添加到顶点上,这和在数学中学到的不一样: 点没有方向,所以没办法和点垂直.而 =OpenGL= 会先找出该点的平面,然后计算垂直于该平面的法线变量.

光线的计算需要法线向量变换(normal vector transformation),光线的计算,也叫光照处理(lighting processing),是发生在视点坐标系下的.

所以法线向量变换也是发生在从对象坐标到视点坐标变换(the modelview matrix)这个过程中的,不过法线向量变换和这变换不一样.

从数学上来说,最好把法线向量看作一个平面,垂直于几个向量(最少3个,3个顶点就能够构成一个平面,少1个不行,多1个可能不行),

那么法线向量的变换就是垂直平面的变换了.

假设现在有一个平面 $v_{1}-v_{2}-v_{3}$ (三个顶点够构成一个平面,多一个或者少一个都不行),它的法线向量的齐次坐标形式是 $\vec{n} = \left(n_{x}, n_{y}, n_{z}, n_{w}\right)$,

欧几里得形式则是 $\left(n_{x}, n_{y}, n_{z}\right)$.如图所示,

[[../../../files/gl_normaltransform02.png]]

在齐次坐标系下,平面是用 $\left(a, b, c, d\right)$ 这种形式表示的,而 $a$, $b$, $c$, $d$ 都不为 0.

其中 $a, b, c$ 代表法线向量的分量,也是平面的方向, $d$ 表示平面距离原点的距离.

如果 $q$ 不为 0,那么 $\left(qa, qb, qc, qd\right)$ 和 $\left(a, b, c ,d\right)$ 表示同一个平面.

如果有一个点 $\left(x, y, z, w\right)$ 满足关系: $ax + by + cz + dw = 0$,那么该点就在该平面上,

如果 $w = 1$,那么就是标准的欧几里得平面方程了.
