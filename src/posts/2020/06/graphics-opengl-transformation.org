...
不过要记住, OpenGL 里面会按照相反的指令顺序执行变换的, 所以如果是想按照 *缩放,旋转和平移* 这个顺序进行变换,

那么代码里面就要按照 *平移,旋转和缩放* 这个顺序写. 总的来说, 模型变换本质上还是几何变换/坐标变换.
#+END_QUOTE
...
#+begin_quote
这并非唯一的构建方案, 本文选择这种方案是基于视点变换是基底变换这个原因.
...
那么可以通过该计算得出法线方向: $M_{model} \left(\begin{array}{c} n_x \\ n_y \\ n_z \\ n_w \end{array} \right)$;

如果 $M_{model}$ 只包含了等比缩放和旋转和平移, 那么 $(M_{model}^{-1})^{T} \ne M_{model}$, 那么可以这么完成法线计算: $M_{model} \left(\begin{array}{c} n_x \\ n_y \\ n_z \\ n_w = 0 \end{array} \right)$,

原理是: 只包含等比缩放和旋转意味着 $M_{model}$ 必然是正交矩阵, 所以 $M_{model}^{-1} = M_{model}^{T}$,

而 $n_w = 0$ 可以消除平移变换带来的影响, 可以看作把 $M_{model}$ 降低了一维, 这一维正好对应平移变换.
#+END_QUOTE
