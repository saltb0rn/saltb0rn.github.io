...
=OpenGL= 有一个叫做 $lookAt$ 的变换,就是用来对相机进行变换的.

它是由同名函数构建的, 这个函数参数是三个世界坐标系上的向量:

- $eye$ - 指定相机的位置
- $center$ - 相机看向的位置
- $up$ - 指定相机向上的方向

之前也说了, =OpenGL= 的相机是虚拟的,它是根据参考的相对性对场景进行逆向变换来达到"变换相机"的效果.
...
$lookAt$ 变换就是相机从位于原点上看着(右手坐标系的) $z$ 的负方向, 到 $eye$ 的位置上看着 $center$.
...
首先是平移,因为是场景的反向平移,所以 $M_{T} = \left(\begin{array}{c} 1 & 0 & 0 & -x_{t} \\ 0 & 1 & 0 & -y_{t} \\ 0 & 0 & 1 & -z_{t} \\ 0 & 0 & 0 & 1\end{array}\right)$.
...
假设相机在作为场景对象时的变换矩阵是 $M_{r} = \left(\begin{array}{c} r_{x} & u_{x} & f_{x} & 0 \\ r_{y} & u_{y} & f_{y} & 0 \\ r_{z} & u_{z} & f_{z} & 0 \\ 0 & 0 & 0 & 1 \end{array}\right)$, 那么 $M_{R} = M_{r}^{-1} = M_{r}^{T} = \left(\begin{array}{c} r_{x} & r_{y} & r_{z} & 0 \\ u_{x} & u_{y} & u_{z} & 0 \\ f_{x} & f_{y} & f_{z} & 0 \\ 0 & 0 & 0 & 1\end{array}\right)$.

(由于 $M_{r}$ 是一个旋转矩阵, 而旋转矩阵是[[https://en.wikipedia.org/wiki/Orthogonal_matrix][正交矩阵]], 正交矩阵的逆矩阵就是它的转置矩阵).

以相机作为原点建立一个正交坐标系,只要求出该坐标系的三个基底变量就可以解得该矩阵.

首先, 得到相机的面向 $forward = center - eye$, 并求出 $forward$ 的单位向量 $\hat{f} = (f_x, f_y, f_z)$;

然后, 用 $\hat{f}$ 或者 $forward$ 与向量 $up$ 进行叉积, 求出同时垂直于它们的向量 $right = \hat{f} \times up$, 从而得出其单位向量 $\hat{r} = (r_x, r_y, r_z)$;

最后, 对 $\hat{f}$ 和 $\hat{r}$ 进行叉积求出同时垂直于它们的向量 $\hat{u} = \hat{f} \times \hat{r} = (u_x, u_y, u_z)$.

至此, 三个基底变量就求出来了.

#+BEGIN_QUOTE
问什么要用 $up$ 和 $\hat{f}$ 求出 $\hat{r}$ 后还得反过来让 $\hat{f}$ 和 $\hat{r}$ 进行叉积求出 $\hat{u}$, 而不是直接对 $up$ 进行规范化得到 $\hat{u}$ 呢?

这是因为我们建立的矩阵 $M_{r}$ 是正交坐标系, 也就是 $\hat{r}/\hat{u}/\hat{f}$ 三者是相互垂直的,

然而 $up$ 和 $\hat{f}$ 只是可以构成一个平面, 但不一定垂直.

另外, 由于 $\hat{f}$ 和 $\hat{r}$ 都是单位向量, $\hat{u} = \hat{f} \times \hat{r}$ 可以看作 $\hat{f}$ 围绕 $\hat{r}$ 往逆时针方向旋转 $90^{\circ}$ 到达方向 $\hat{u}$.

因此, 不需要对 $\hat{f} \times \hat{r}$ 求单位向量.
#+END_QUOTE
...
#+BEGIN_SRC js
  ...
  const top = 2.5
  const bottom = -2.5
  const orthoFrustumHeight = top - bottom
  // 正交视椎体高度, 第二个重点
  const left = orthoFrustumHeight * aspect / -2
  const right = orthoFrustumHeight * aspect / 2
  // 根据 aspect 计算 left 和 right, 使得最终成像的比例和 3D 成像的比例一致, 第三个重点
...
其中 =left/right= 是 =2D UI= 画布在水平方向的范围, =top/bottom= 则是在垂直方向上的范围, 在绘制 =UI= 元素时需要根据这个坐标系来对元素进行定位.
