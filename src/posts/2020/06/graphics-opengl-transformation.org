...
并让这 6 个参数要满足这样的关系 $\begin{equation*} \left\{ \begin{aligned} & l < r \\ & b < t \\ & 0 < n < f\end{aligned} \right. \end{equation*}$, 这里只让 $n$ 和 $f$ 大于 0 是因为相机看的方向是 $-Z$.
...
这里第一个就是透视的截头锥体, 第二个是标准化设备坐标系, 两者的 $Z$ 轴方向是相反的.

为了方面说明推导过程, 先声明一些变量:

由于相机看向的是 $-Z$ 方向, 所以近和远裁剪平面到相机的距离 $-n$ 和 $-f$,
...
就是把世界坐标的 $z_{m}$ 分量从 $[n, f]$ 重映射到 $[0, 1]$, 步骤如下:
...
$k_z$ 就有点特殊, 因为相机看向 $-Z$, 所以是从 $[-n, -f]$ 映射到 $[-1, 1]$, 经过计算得到 $k_z = \frac{2}{n - f}$ 以及 $c_z = \frac{n + f}{n - f}$.
