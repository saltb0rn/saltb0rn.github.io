...
在上述等式中, 可以把 $\frac{1}{\frac{1}{f} - \frac{1}{n}}$ 看作是一个常数 $k$, $d$ 看作是以 $\frac{1}{z_e}$ 为参数的函数 $d(\frac{1}{z_e}) = k(-\frac{1}{z_e} - \frac{1}{n})$,

可以发现, $d$ 不仅不与 $z_{e}$ 成线性关系, 反而与 $\frac{1}{z_{e}}$ 成线性关系, 这就是为什么 $d$ 被称为非线性深度.
...
-----
*不止于深度* @@html: <a id="arbitrary-attr-interp"></a>@@
