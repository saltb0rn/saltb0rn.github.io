...
#+tags: Graphics
#+begin_abstract
之前只讲了几何变换,也就是最简单的变换, =OpenGL= 里面还有很多其它变换,本文主要讲述 =OpenGL= 的各种坐标系以及它们之间的转换,这对于理解 =OpenGL= 的渲染管线/渲染流水线(rendering pipeline)很有必要.

关于渲染管线这部分的内容也不打算讲述了,这里有一篇文章讲的很清晰了: [[http://www.songho.ca/opengl/gl_pipeline.html][OpenGL Rendering Pipeline]],下面的内容都是参考从这位大佬的 [[http://www.songho.ca/opengl/gl_transform.html][OpenGL Transformation]] 写的.

这篇文章就是关于渲染管线中的 =Vertex Operation= 以及 =Primitive Assembly= 两个环节中涉及的数学原理.
...
*第一步* 是就是选择相机看到的内容,因为相机是不能看到完整场景的(不可能看到无限远),所以要先求出相机看到空间范围,因此需要求出哪些顶点是在视野范围内的,
...
*第三步*,就是把裁剪空间里面的内容转换成像素,这一步叫光栅化(Raterization),也就是平时常说的"矢量图转像素图",这个时候已经是 2D 的了.

*这已经是 =Primitive Assembly= 下一个环节了,之所以写它,那么是为了加深对 =NDC= 的理解,不过还有另外一个原因(在结尾处我会解释).*

这一步其实就是要计算出像素在视口中的位置,如何描述像素的位置呢?这就需要一个概念叫做窗口坐标(window coordinate)/屏幕坐标(screen coordinates)了.
...
1. [[https://www.davrous.com/2013/06/13/tutorial-series-learning-how-to-write-a-3d-soft-engine-from-scratch-in-c-typescript-or-javascript/][Tutorial series: learning how to write a 3D soft engine from scratch in C#, TypeScript or JavaScript]]
   ...
   这个是系列教程的第一个部分: 编写一个软件加速的 3D 引擎的核心部分,相对于整个 =OpenGL= 管线来说,这部分的教程实现了 =Vertex Operation=, =Primitive Assembly= 以及 =Raterization= 这三个环节.

   这就是为什么我要把光栅化这一步也写上去(其实仔细想一下,把光栅化变为本文的一个重要介绍环节不就好了吗?之后有时间再补上吧).这个教程需要读者掌握 =C#=, =TypeScript= 和 =JavaScript= 的其中一门编程语言,

   个人不建议只是读代码的,最好自己动手敲一遍,但是读者一味照"敲"也容易陷入在一些细节上思考不到位的情况,本人是 =JavaScript= 用户,从自己身经验来说,推荐把 =babylon.math.js= 换成 =gl-matrix.js=.

   =gl-matrix.js= 的 =API= 比起 =babylon.math.js= 的 =API= 更加底层和贴近 =OpenGL= 的 =API=.

2. [[http://glmatrix.net/][glMatrix]]

   如果读者是 =JavaScript= 用户,那么本人推荐去阅读以下 =gl-matrix.js= 的源代码,这对于检测线性代数以及矩阵推导过程的理解有帮助,如果理解到位了, =gl-matrix.js= 源代码就很好懂.
