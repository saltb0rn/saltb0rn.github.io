...
我们很容易得到 $\left(f_{x}, f_{y}, f_{z}\right)$,也就是从看的目标的位置到相机的方向: $forward = \left(x_{t}, y_{t}, z_{t}\right) - \left(x_{c}, y_{c}, z_{c}\right)$,然后求出 $forward$ 的单位向量 $\hat{f} = (f_x, f_y, f_z)$;

然后把给出的(*如果没给出就用 $\left(0, 1, 0\right)$*)向上方向的向量 $up$ 与 $\hat{f}$ 进行叉积乘法求出同时垂直于它们的向量 $right$, 并且求出其单位向量 $\hat{r} = (r_x, r_y, r_z)$;

最后对 $\hat{f}$ 和 $\hat{r}$ 进行叉积乘法求出同时垂直于它们的向量, 并求出其单位向量 $\hat{u} = (u_x, u_y, u_z)$, 这样三个基底变量就解出了.
