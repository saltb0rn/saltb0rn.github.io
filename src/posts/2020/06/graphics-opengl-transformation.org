...
一开始指定6参数就决定了看到的内容,裁剪判断也很简单,只要点的坐标: $\left(x, y, z, w\right)$ 满足这个关系 $\begin{equation}\left\{\begin{aligned} l < x < r \\ b < y < t \\ n < z < f \end{aligned}\right.\end{equation}$ 就不会被裁剪,也就是说点的 $w$ 不再作为裁剪判定的根据,

所以也不需要特别去计算 $w$ 的值了,也就是说再指定参数的时候裁剪处理已经完成了.

首先是从裁剪坐标转换成 $ndc$ 就是单纯地分别就计算出 $x_{p}$, $y_{p}$ 以及 $z_{p}$ 到 $x_{n}$, $y_{n}$ 以及 $z_{n}$ 是线性关系.
