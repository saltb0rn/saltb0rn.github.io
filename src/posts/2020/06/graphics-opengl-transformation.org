...
#+tags: Graphics
#+begin_abstract
之前只讲了几何变换,也就是最简单的变换,本文主要透过 =OpenGL= 的各种坐标系来讲解图形学的渲染管线的坐标系是如何转换的,这对于理解 =OpenGL= 的渲染管线/渲染流水线(rendering pipeline, 我也不知道为什么会这么翻译)很有必要.
...
假设有一个表达式 $P = \left(x, y, z\right)$, *在没有特殊说明下*, 人们无从得知 $P$ 是点还是向量, 因为点和向量的表示都一样.

因此人们添加 $w$ 作为标识用来区分 $P$ 是点还是向量: $P = \left(x, y, z, w\right)$.

当 $w = 0$ 时, $P$ 就是向量, 否则 $P$ 就是点.
#+END_QUOTE
...
我们假设现在在该坐标系下有一个点 $p_{o} = \left(x_{o}, y_{o}, z_{o}\right)$,
...
假设相机的变换矩阵是 $M_{r} = \left(\begin{array}{c} l_{x} & u_{x} & f_{x} & 0 \\ l_{y} & u_{y} & f_{y} & 0 \\ l_{z} & u_{z} & f_{z} & 0 \\ 0 & 0 & 0 & 1 \end{array}\right)$, 那么 $M_{R} = M_{r}^{-1} = M_{r}^{T} = \left(\begin{array}{c} l_{x} & l_{y} & l_{z} & 0 \\ u_{x} & u_{y} & u_{z} & 0 \\ f_{x} & f_{y} & f_{z} & 0 \\ 0 & 0 & 0 & 1\end{array}\right)$.
...
我们很容易得到 $\left(f_{x}, f_{y}, f_{z}\right)$,也就是从看的目标的位置到相机的方向: $forward = \left(x_{t}, y_{t}, z_{t}\right) - \left(x_{c}, y_{c}, z_{c}\right)$,然后求出 $forward$ 的单位向量 $f$;

然后把给出的(*如果没给出就用 $\left(0, 1, 0\right)$*)向上方向的向量 $up$ 与 $f$ 进行叉积乘法求出同时垂直于它们的向量 $left$, 并且求出单位向量 $l$;

最后对 $f$ 和 $l$ 进行叉积乘法求出同时垂直于它们的向量 $u$,这样三个基底变量就解出了.

#+BEGIN_QUOTE
问什么要用 $up$ 和 $f$ 求出 $l$ 后还得反过来让 $f$ 和 $l$ 进行叉积求出 $u$, 而不是直接对 $up$ 进行规范化得到 $u$ 呢?

这是因为我们建立的矩阵 $M_{r}$ 是正交坐标系, 也就是 $l/u/f$ 三者是相互垂直的, 然而 $up$ 和 $f$ 只是可以构成一个平面, 但不一定垂直.
#+END_QUOTE
...
同样, $M_{modelview}$ 里面的缩放变换是等比缩放的(也就是 $x$, $y$, $z$ 各个分量的缩放系数一致), 只会影响法线长度, 不影响发现的方向, 也是可以一同忽略.
...
比如 =NDC= 的 $x_{n}$ 分量就是裁剪坐标 $x_{clip}$ 与视口宽度的 *一半* 的比例,这个比例乘以视口的尺寸就可以计算出点在视口中的实际位置.
...
假设投影矩阵 $M_{projection} = \left(\begin{array}{c} x_{l} & x_{u} & x_{f} & x \\ y_{l} & y_{u} & y_{f} & y \\ z_{l} & z_{u} & z_{f} & z \\ w_{l} & w_{u} & w_{f} & w \end{array}\right)$,那么变换过程就是: $p_{p} = \left(\begin{array}{c} x_{p} \\ y_{p} \\ z_{p} \\ w_{p} \end{array}\right) = M_{projection}\left(\begin{array}{c}x_{e} \\ y_{e} \\ z_{e} \\ w_{e}\end{array}\right)$,

把裁剪后剩下的顶点变换成 =NDC=, $p_{n} = \left(\begin{array}{c}x_{n} \\ y_{n} \\ z_{n}\end{array}\right) = \left(\begin{array}{c}\frac{x_{p}}{w_{p}} \\ \frac{y_{p}}{w_{p}} \\ \frac{z_{p}}{w_{p}} \end{array}\right)$.

下图展示了一个视点空间上的点 $p_{e}$ 如何投影到近裁剪平面的点 $p_{p}$ 上.
