#+title: 图形学 - OpenGL坐标系变换
#+date: 2020-06-19
#+index: 图形学 - OpenGL坐标系变换
#+tags: Graphics
#+options: auto-id:t
#+begin_abstract
...
** 几个该清楚的概念
:PROPERTIES:
:CUSTOM_ID: h-08a9bbb6-87bd-4499-bb6a-f2bcd7af4918
:END:

*** 齐次空间
:PROPERTIES:
:CUSTOM_ID: h-d94e2b1c-531b-4763-9627-7b5267b0c950
:END:
...
** OpenGL 中对象的变换历程
:PROPERTIES:
:CUSTOM_ID: h-483037b0-7c15-42d8-bfb8-1490821da4ed
:END:
...
*** 对象坐标系
:PROPERTIES:
:CUSTOM_ID: h-91530c69-157a-4c03-af73-d8fa077e0ee4
:END:

=3D= 模型文件的顶点(vertex)数据所使用的坐标系就叫做对象坐标系(object coordinate system).

每个模型都有独属自己的对象坐标系, *对于一个模型的顶点来说*, 它的对象坐标系是固定不变的,

这就是为什么对象坐标系也被叫做局部坐标系(local coordinate system).

永远都是以 $(0, 0, 0)$ 为原点, $x/y/z$ 三轴正交, 坐标轴的指向因图形软件而不同,

本文的是 $X$ 轴向右, $Y$ 轴向上, $Z$ 轴向屏幕外.

假设有一个模型的顶点 $p_{o} = \left(x_{o}, y_{o}, z_{o}\right)$, 接下来会探讨一下这个点会经历过哪些变化.

*** 世界坐标系
:PROPERTIES:
:CUSTOM_ID: h-8d15afa4-8baf-48d1-970a-9e9b00ebe19c
:END:

在 =3D= 的世界中, 可以使用若干个模型可以构成一个复杂的模型, 然后又可以使用若干个复杂模型构成更加复杂的模型.

我们把那个最复杂的模型称为场景(scene)或者世界(world), 这个模型的对象坐标系被称为世界坐标系(world coordinate system).

对于场景来说, 世界坐标系本质上就是它的对象坐标系; 对简单模型来说, 世界坐标系是全局的,

因此世界坐标系又被称为全局坐标系(global coordinate system), 并且是固定不变的:

以 $(0, 0, 0)$ 为原点, $x/y/z$ 三轴正交且指向固定方向.

*** 从对象坐标系到世界坐标系的变换
:PROPERTIES:
:CUSTOM_ID: h-4d15c6d7-b964-46c7-b2fa-cc1a9a7d54f1
:END:

图形软件加载模型时, 模型会被放置在世界坐标系的原点上.

在模型没有经过旋转/平移/缩放等变换时, 模型的对象坐标系和世界坐标系是一样的.

#+attr_html: :width 600px
#+caption: 对模型进行平移后, 模型的对象坐标系和世界坐标系的关系
[[../../../files/global-vs-local-coordinate.png]]

#+attr_html: :width 600px;
#+caption: 先旋转再平移模型, 模型的对象坐标系和世界坐标系的关系
[[../../../files/global-vs-local-coordinate-2.png]]

在渲染模型成像的过程中, 需要计算出顶点坐标 $p_{o}$ 在世界坐标系上的坐标 $p_{m}$.

从图形中可以看出, 这本质上就是求 $p_{o}$ 的坐标变换结果, 坐标变换的内容在 [[./graphics-geometric-transformation.html][图形学 - 几何变换]] 中有详细讲解, 就不赘述了.

*从对象坐标系到世界坐标系的变换称为做模型变换(modeling transformation)*,
...
其中 $\left(\begin{array}{c}m_{1x} \\ m_{1y} \\ m_{1z}\end{array}\right)$, $\left(\begin{array}{c}m_{2x} \\ m_{2y} \\ m_{2z}\end{array}\right)$ 以及 $\left(\begin{array}{c}m_{3x} \\ m_{3y} \\ m_{3z}\end{array}\right)$ 分别是对象坐标系的 $x$, $y$ 以及 $z$ 轴,

而 $\left(\begin{array}{c}d_{x} \\ d_{y} \\ d_{z} \end{array}\right)$ 是对象坐标系原点的在世界坐标系上的坐标.
...
最后, $p_{m} = M_{model}p_{o} = \left(x_{m}, y_{m}, z_{m}\right)$.

*** 视点坐标系
:PROPERTIES:
:CUSTOM_ID: h-d956954f-12dd-4c6e-a8b3-f63e88b2b7f2
:END:
...
视点变换和前面的模型变换不同, 视点变换属于[[./graphics-geometric-transformation.html#the-way-to-find-transformations][基底变换]], 这种变换不会修改顶点的位置, 只会修改坐标系,

这也能解释为什么视点变换是相机模型变换的逆变换, 因为基底变换与坐标变换就是互逆变换.
...
最后得到 $p_{m}$ 在视点坐标系上的坐标 $p_{e} = M_{view}p_{m} = \left(x_{e}, y_{e}, z_{e}\right)$.

**** 法线向量变换
:PROPERTIES:
:CUSTOM_ID: h-35374513-d847-4421-92d0-f240f025245f
:END:
...
# 这是因为在现实世界中表面不可能是平的,放大看会非常粗糙,这样的凹凸不平的表面上可以找到无数个不同方向的法线,

# 非常乱并且运算量十分庞大, 而在计算机中所有面可以说是平的,哪怕曲面都是由多个小平面堆砌而成的近似结果,

# 越是放大曲面就越能看到它的小平面,这对应现实中一个例子:地球,行走在地面上感觉地面是平的,但是通过卫星却看到地球是圆的,

# 计算机中的曲线也一样的,用图形处理软件放大像素图中的圆形可以看到它的边是由多条短小直线构成的.

# 法线都是垂直于这些小平面的,由于小平面是由顶点构成的,所以法线向量就很合理地成为顶点的属性.

比如 [[http://math.hws.edu/graphicsbook/c4/s1.html][graphicsbook]] 这里的例子:
...
我推荐通过 [[https://learnopengl.com/Getting-started/Textures][LearnOpenGL - Textures]] 这篇文章来了解前面提到的几个概念就足以, 不用看太多.
...
切线空间是模型平面上的一个空间. 而因用途不同, 切线空间也分类型, 当然它们的构建方式也不一样.

比如 $n_{texture}$ 所处于的就是纹理切线空间. 不管是哪种切线空间,

*切线空间坐标系与对象坐标系之间的关系* 就如 *对象坐标系与世界坐标系之间的关系*: 局部对全局.

正如开头所说的, $n_{texture}$ 不一定与顶点所处的平面垂直,

所以无法像面发现 $n_{plane}$ 与顶点之间存在约束关系, 可以使用 $M_{model}^{-1}$ 进行变换.

为此, 我们需要找出一个变换:

先让 $n_{texture}$ 从切线空间变换到对象坐标系上得到 $n_{o}$, 这属于[[./graphics-geometric-transformation.html#the-way-to-find-transformations][基底变换]],

再让 $n_{o}$ 随着顶点经过 $M_{model}$ 变换后得出世界坐标系上的表示 $n_{m}$.

# 因此 $n_{texture}$ 不像 $n_{plane}$ 那样与顶点所处的平面垂直, 但切线空间是根据顶点构建出来的,

这个从切线空间坐标系到世界坐标系的变换 $M_{tbn}$ 被称为 =TBN Matrix=.

=TBN= 分别是取自切线空间坐标轴 =Tangent=, =Bitangent= 和 =Normal= 的首字母.

由于 $n_{texture}$ 是根据纹理坐标获取的, 因此要以纹理坐标系作为变换的起点.

#+CAPTION: 三角形俯视图 - 三角形顶点与其纹理坐标的对应关系
[[../../../files/surface-texture-coordinate-system.png]]

假设现在有一个由顶点 $A_{0}$, $A_{1}$ 和 $A_{2}$ 构成的三角形, 而 $n_{plane}$ 是它的面法线,

$T = (x_{T}, y_{T}, z_{T})$ 和 $B = (x_{B}, y_{B}, z_{B})$ 分别切线空间的基向量,

对象坐标系中的向量可以表达为纹理坐标差 $\Delta u$ 和 $\Delta v$ 对 $T$ 和 $B$ 的线性组合:
...
在计算出 $T$ 和 $B$ 后就可以计算出 $N = (x_{N}, y_{N}, z_{N}) = T \times B = ( y_{T} z_{B} - z_{T} y_{B}, z_{T} x_{B} - x_{T} z_{B}, x_{T} z_{B} - z_{T} x_{B} )$.
...
然而最开始的基向量 $T$ 和 $B$ 并没有保证过是正交的, 为了最后得出正交的 $M_{tbn}$,

需要通过 *格拉姆-施密特处理(Gram-Schmidt process)* 让 $M^{'}_{tbn}$ 进行正交化, 通俗点说就是让 $T^{'}$, $B^{'}$ 和 $N^{'}$ 三者相互垂直.

因为 $N^{'}$ 是同时垂直于 $B^{'}$ 和 $T^{'}$ 的, 所以以 $N^{'}$ 为基准重新对齐 $T^{'}$ 得到 $T^{''}$: $T^{''} = \frac{T^{'} - (T^{'} \cdot N^{'}) \times N^{'}}{|T^{'} - (T^{'} \cdot N^{'}) \times N^{'}|}$,

$B^{'}$ 的对齐可以直接通过 $N^{'}$ 和 $T^{''}$ 的叉乘求出得到 $B^{''}$: $B^{''} = \frac{N^{'} \times T^{''}}{|N^{'} \times T^{''}|}$.

到目前为止, 我们已经构建出切线空间的坐标系了.

由于 $M_{tbn}$ 是从切线空间到世界坐标系的变换, 所以还需要对三个基向量进行 $M_{model}$ 变换,
...
:PROPERTIES:
:CUSTOM_ID: h-2c049853-8690-4db2-8319-bd37bbf2fca9
:END:

*** 3D转化成2D图像
:PROPERTIES:
:CUSTOM_ID: h-f1f030d6-c287-43c6-9834-8099243f1039
:END:
...
:PROPERTIES:
:CUSTOM_ID: h-4fc5321b-c9b4-4007-9080-ec29e6b81543
:END:
...
**** 透视投影
:PROPERTIES:
:CUSTOM_ID: h-85469068-5c60-40f4-91d2-c2be1aaf9a2e
:END:
...
根据这个关系可以得到 $\frac{x_{p}}{x_{e}} = \frac{-n}{z_{e}}$, 所以 $x_{p} = \frac{-nx_{e}}{z_{e}} = \frac{nx_{e}}{-z_{e}}$, 这么一来 $p_e$ 就被投影到近裁剪平面上了.
...
还有为了反转 $z$ 轴的正负方向, 可以把 $w_{p}$ 取为 $-z_{e}$,
...
既然如此,那么 $x_{p}$ 以及 $y_{p}$ 是不是可以分别取 $\frac{nx_{e}}{-z_e}$ 以及 $\frac{ny_{e}}{-z_e}$ 了吗?

#+begin_quote
一些简单的渲染引擎会使用这样的投影公式 $f(p = (x, y, z), d) = (\frac{x \cdot d}{z}, \frac{y \cdot d}{z})$ 实现投影,

其中 $p$ 是世界坐标或视点坐标, $d$ 是相机和投影平面之间的距离, $f$ 的结果 $c$ 是投影平面上的一个坐标点,

这是实现透视效果的最简单模型. 但是这个模型有一个问题, 坐标 $c$ 的分量范围是不固定的, 这不方便于储存, 也难以处理.

这就是 =OpenGL= 的透视投影不采用这个模型的原因: 投影坐标需要被限定在一个范围内.

可以肯定的是, =OpenGL= 透视投影的核心思想就是这个投影公式.
#+end_quote
...
# *再观察 $z_{n}$ 以及 $z_{e}$ 的关系可以发现它们根本就不成线性关系,且成反比关系*: $z_{e}$ 越大, $z_{n}$ 越小.

# 这意味着在裁剪近平面附近的顶点的精准度会很高,在裁剪远平面附近的顶点精准度会很低.
...
- [[../../../files/lowk_persp_interp_techrep.pdf][=Perspective-Correct Interpolation=]]
...
(PS: 其实 [[https://registry.khronos.org/OpenGL-Refpages/gl4/html/glDepthRange.xhtml][glDepthRange(a, b)]] 可以改变这个映射范围到 $[a, b]$: $a + (a - b) \times d$, 但是为了简化下面的计算推导, 我选择最简单的范围: $[0, 1]$.)
...
透视投影所产生的 $d$ 被称为非线性深度(=non-linear depth=),

=OpenGL= 中的线性深度(=linear depth=)是指深度指 $d$ 与视点坐标的 $z$ 分量 $z_{e}$ 成线性关系: $d = k \cdot z_{e} + c$, 其中 $k, c$ 为任意常数.

通俗点说就是 $z_{e}$ 每增加或减少一个单位 $\Delta$ 时: $z_{e} \pm \Delta$, $d$ 都会以固定比例 $k$ 发生变化: $d \pm k \cdot \Delta$;

否则 $d$ 与 $z_{e}$ 成非线性关系. 接下来会通过公式变换来展示出 $d$ 与 $z_{e}$ 成非线性关系:

$\begin{equation*} \begin{aligned} d &= (z_{n} + 1) \div 2 \\
&= \left( \frac{-\frac{f+n}{f-n}z_{e} - \frac{2fn}{f-n}}{-z_{e}} + 1 \right) \div 2 \\
&= \left( \frac{\frac{-z_{e}(f+n) - 2fn}{f-n}}{-z_{e}} + 1 \right) \div 2 \\
&= \left( \frac{-z_{e}(f+n) - 2fn}{-z_{e}(f-n)} + 1 \right) \div 2 \\
&= \left( \frac{-z_{e}(f+n) - 2fn}{-z_{e}(f-n)} + \frac{-z_{e}(f-n)}{-z_{e}(f-n)} \right) \div 2 \\
&= \frac{2\times -z_{e}f - 2fn}{-z_{e}(f-n)} \div 2 \\
&= \frac{-z_{e}f - fn}{-z_{e}(f-n)} \\
&= \frac{f(z_{e} + n)}{z_{e}(f-n)} \\
&= \frac{f}{f - n} + \frac{1}{z_{e}} \frac{n}{f - n} && \text{这里就可以看出 }d\text{与 }\frac{1}{z_{e}}\text{ 成线性关系}\\
&= \frac{n + z_{e}}{z_{e}n} \times \frac{fn}{f-n} \\
&= \frac{n + z_{e}}{z_{e}n} \div \frac{f-n}{fn} \\
&= (\frac{1}{z_{e}} + \frac{1}{n}) \div (\frac{1}{n} - \frac{1}{f}) \\
&= \frac{\frac{1}{z_{e}} + \frac{1}{n}}{\frac{1}{n} - \frac{1}{f}} \\
&= \frac{-\frac{1}{z_{e}} - \frac{1}{n}}{\frac{1}{f} - \frac{1}{n}} \\
&= \frac{1}{\frac{1}{f} - \frac{1}{n}} \left( -\frac{1}{z_{e}} - \frac{1}{n} \right)
\end{aligned} \end{equation*}$

在上述等式中, 可以把 $\frac{1}{\frac{1}{f} - \frac{1}{n}}$ 看作是常数 $k$, 把 $k \cdot \left( - \frac{1}{n} \right)$ 看作常数 $c$, 把 $d$ 看作是以 $z_e$ 为参数的函数 $d(z_e) = k(-\frac{1}{z_e}) + c$.

可以发现, $d$ 不仅不与 $z_{e}$ 成线性关系, 反而与 $\frac{1}{z_{e}}$ 成线性关系.

有些效果需要使用线性深度, 相比非线性深度反映物体与相机在视觉中的距离, 线性深度更能反映物体与相机之间在世界中的距离,

线性深度 $d_{linear}$ 的定义如下:
...
- 方法一:
  ...
  最终整个计算过程为 $p_{e} = \frac{p^{'}_{e}}{(p^{'}_{e})_w} = \frac{M^{-1}_{projection} p^{'}_{n}}{ (M^{-1}_{projection} p^{'}_{n})_w } = \left(\begin{array}{c} \frac{(p^{'}_{e})_x}{(p^{'}_{e})_w} \\ \frac{(p^{'}_{e})_y}{(p^{'}_{e})_w} \\ \frac{(p^{'}_{e})_z}{(p^{'}_{e})_w} \\ \frac{(p^{'}_{e})_w}{(p^{'}_{e})_w} = 1 \end{array}\right)$.
...
- 方法二:
  ...
  & z_{n} \\
  = & \frac{z_{p}}{w_{p}} \\
  = & \frac{-\frac{f+n}{f-n}z_{e} - \frac{2fn}{f-n}}{-z_{e}} \\
  = & \frac{f+n}{f-n} + \frac{2fn}{f-n} \times \frac{1}{z_{e}} \\
  \rightarrow & z_{n}(f - n) = (f + n) + \frac{2fn}{z_{e}} \\
  \rightarrow & \frac{2fn}{z_{e}} = z_{n}(f - n) - (f + n) \\
  \rightarrow & z_{e} = \frac{2nf}{z_{n}(f - n) - (f + n)}
  \end{aligned} \end{equation*}$

  也可以把 $z_n$ 转换成非线性深度 $d = z_n \times 2 - 1$, 然后直接从 $d$ 反推 $z_e = \frac{nf}{(f - n) \times d - f}$.

-----
*从深度值计算出发, 学习在透视投影下的属性插值计算* @@html: <a id="arbitrary-attr-interp"></a>@@

把 $d = \frac{-\frac{1}{z_e} - \frac{1}{n}}{\frac{1}{f} - \frac{1}{n}}$ 变换一下可以得到一个形式符合线性插值定义的等式:

$\begin{equation*}\begin{aligned} -\frac{1}{z_e} & = \frac{1}{n} + d \times (\frac{1}{f} - \frac{1}{n}) \\
\rightarrow \frac{1}{z_e} & = -\frac{1}{n} + d \times (\frac{1}{n} - \frac{1}{f}) \\
& = -\frac{1}{n} + d \times \left[\left(-\frac{1}{f}\right) - \left(-\frac{1}{n}\right)\right]
\end{aligned}\end{equation*} d \in [0, 1]$

回想一下, $z_{p} \in [-n, -f]$, 所以 $-\frac{1}{f}$ 和 $-\frac{1}{n}$ 分别是远近裁剪平面在裁剪空间中的 $z$ 分量的倒数,

根据线性插值的定义, 可发现 $-\frac{1}{f}$ 和 $-\frac{1}{n}$ 分别是视锥体范围内视点坐标的 $z_e$ 的倒数 $\frac{1}{z_e}$ 的最大和最小值,

即 $\frac{1}{z_e} \in [-\frac{1}{n}, -\frac{1}{f}]$, 由于 $0 \lt n \lt f$, 可以得出 $z_e \in [-f, -n]$.

我们可以把这个插值函数推广到任意范围 $z_e = [Z_{p1}, Z_{p2}]$ 上,

用来求任意两个裁剪坐标 $\begin{cases} p1 = (X_{p1}, Y_{p1}, Z_{p1}) \\ p2 = (X_{p2}, Y_{p2}, Z_{p2}) \end{cases}$ 中间某个裁剪坐标所对应的视点坐标 $z$ 分量 $z_e$ 的倒数 $\frac{1}{z_e}$:

$\frac{1}{z_e} = f(t) = \frac{1}{Z_{p1}} + t \times \left(\frac{1}{Z_{p2}} - \frac{1}{Z_{p1}}\right), t \in [0, 1]$.

这正是透视投影公式 $f(p = (x, y, z)) = ( \frac{x \cdot d}{z}, \frac{y \cdot d}{z} )$ 用来插值获得两片元中间某个片元深度值的方法,

这个经典的模型使用世界坐标或视点坐标 $p$ 的 $z$ 分量作为深度值, $d$ 是投影平面 (近裁剪平面) 与相机之间的距离.

能够通过投影矩阵推导出这个插值函数, 说明投影矩阵与这条投影公式的本质是一样的,

与直接使用视点坐标的 $z$ 分量作为深度值不同, 投影矩阵计算出插值函数 $\frac{1}{z_e} = f(t)$ 的参数 $t = f^{-1}(\frac{1}{z_e})$ 作为深度值,

由于后者的深度值在限定范围中, 因此非常适合储存和用于编程处理.

我们可以更进一步, 通过插值函数 $f(t)$ 可以快速得到视点坐标的 $z$ 分量 $z_e = \frac{1}{f(t)}$:

$\begin{equation*} \begin{aligned} & z_e \\ = & \frac{1}{\frac{1}{Z_{p1}} + t \times \left( \frac{1}{Z_{p2}} - \frac{1}{Z_{p1}} \right) } \\
= & \frac{1}{ \frac{1}{Z_{p1}} + t \times \frac{Z_{p1} - Z_{p2}}{Z_{p1} Z_{p2}} } \\
= & \frac{1}{ \frac{Z_{p2}}{Z_{p1}Z_{p2}} + t \times \frac{Z_{p1} - Z_{p2}}{Z_{p1} Z_{p2}} } \\
= & \frac{1}{ \frac{Z_{p2} + t \times (Z_{p1} - Z_{p2})}{Z_{p1}Z_{p2}} } \\
= & \frac{Z_{p1}Z_{p2}}{Z_{p2} + t \times (Z_{p1} - Z_{p2})}
\end{aligned} \end{equation*}$

因为视点空间上的变化是线性的, 包括坐标位置以及坐标关联的其它属性,

假设裁剪坐标 $p1$ 和 $p2$ 所对应的视点坐标分别为 $p_{e1}$ 和 $p_{e2}$,

那么存在线性插值函数 $p_{e} = (x_e, y_e, z_e) = p_{e1} + s \times (p_{e2} - p_{e1}), s \in [0, 1]$,

只要知道其中一个分量的插值规律, 便能推广到其它分量上, 以 $z_e$ 的插值为例:

$\begin{equation*} \begin{aligned} & z_e \\
= & Z_{p1} + s \times (Z_{p2} - Z_{p1}) \\
= & \frac{Z_{p1}Z_{p2}}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} \\
\rightarrow & s \\
= & \left( \frac{Z_{p1}Z_{p2}}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} - Z_{p1} \right) \times \frac{1}{Z_{p2} - Z_{p1}} \\
= & \left( \frac{Z_{p1}Z_{p2}}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} - \frac{Z_{p1}(Z_{p2} + t \times (Z_{p1} - Z_{p2}))}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} \right) \times \frac{1}{Z_{p2} - Z_{p1}} \\
= & \frac{Z_{p1}(Z_{p2} - Z_{p2} - t \times (Z_{p1} - Z_{p2}))}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} \times \frac{1}{Z_{p2} - Z_{p1}} \\
= & \frac{Z_{p1}(- t \times (Z_{p1} - Z_{p2}))}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} \times \frac{1}{Z_{p2} - Z_{p1}} \\
= & \frac{t \times Z_{p1}}{Z_{p2} + t \times (Z_{p1} - Z_{p2})} \\
= & \frac{t \times Z_{p1}}{(1 - t) \times Z_{p2} + t \times Z_{p1}}
\end{aligned} \end{equation*}$

最终得到 $p_e = p_{e1} + \frac{t \times Z_{p1}}{(1 - t) \times Z_{p2} + t \times Z_{p1}} \times (p_{e2} - p_{e1}), t \in [0, 1]$,

并且这个关系还可以推广到 $p_{e1}$ 和 $p_{e2}$ 的其它关联属性上, 假设 $I_1$ 和 $I_2$ 分别是 $p_{e1}$ 和 $p_{e2}$ 关联的属性,

那么该插值函数成立: $I = I_1 + \frac{t \times Z_{p1}}{(1 - t) \times Z_{p2} + t \times Z_{p1}} \times (I_2 - I_1), t \in [0, 1]$.

**** 透视投影 - FOV (Field of View)
:PROPERTIES:
:CUSTOM_ID: h-a913a590-aa66-401f-90cd-5cc1d85ded0d
:END:
...
** 接下来
:PROPERTIES:
:CUSTOM_ID: h-869471b1-15fa-4b8b-a3b0-38a2a56aab0d
:END:
