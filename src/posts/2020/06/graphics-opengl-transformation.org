...
在欧几里得空间中,指定任意平面 $P$ 的某个点 $p_{1}$ 位置延伸一条法线向量 $\vec{n}$: $\left(a, b, c\right)$,该法线向量表示了平面 $P$ 的面向方向,而 $p_{1}$ 确定了平面的位置,可以反过来说 $p_{1}$ 和 $\vec{n}$ 确定了 $P$,

可以想像成一根棒子 $\vec{n}$ 穿过一块有孔 $p_{1}$ 的木板 $P$,如图所示,

[[../../../files/planeGraph01.png]]

然而事实是,没有点和线垂直的这种说法,针对现在的情况就只有线与线条,或者平面与线垂直,又或者平面与平面这些说法.最低限度也要通过线与线表明 $\vec{n}$ 与 $P$ 垂直.

所以需要在 $P$ 上任意再找一个点 $p$ 形成直线: $\vec{p} - \vec{p_{1}} = \left(x - x_{1}, y - y_{1}, z - z_{1}\right)$,
...
如果 $\vec{n}$ 是单位向量,并且 $p_{1}$ 到原点的距离为 $d$, 那么 $p_{1}$ 可以表示成 $\left(da, db, dc\right)$ (\vec{n} 就是原点到 $p_{1}$ 的方向),整个等式变为 $ax + by + cz - d \cdot \left(a^{2} + b^{2} + c^{2}\right) = 0$,

又因为 $\vec{n}$ 是单位向量,所以 $a^{2} + b^{2} + c^{2} = 1$,所以最终等式为: $ax + by + cz - d = 0$.如果 $\vec{n}$ 不是单位向量,那就自己对 $\vec{n}$ 进行一般化,也就是求出它的单位向量,然后进行同样的推导.
