...
关于渲染管线这部分的内容也不打算讲述了,这里有一篇文章讲的很清晰了: [[http://www.songho.ca/opengl/gl_pipeline.html][OpenGL Rendering Pipeline]],下面的内容都是参考这位大佬的 [[http://www.songho.ca/opengl/gl_transform.html][OpenGL Transformation]] 写的.

这篇文章就是关于渲染管线中的 =Vertex Operation= 以及 =Primitive Assembly= 两个环节中涉及的数学推导,也就是 =OpenGL= 在这这两个环节具体做了什么事情.
...
在欧几里得空间中,任意平面 $P$ 的法线向量 $\vec{n}$: $\left(a, b, c\right)$ 表示了平面 $P$ 的面向方向,也就是说, $P$ 上的任意一个点能够确定 $P$ 的位置,

所以一个平面可以靠一个点和平面的法线向量来进行确定.平面 $P$ 上任何一条直线都垂直于 $\vec{n}$,假设这条直线是由 $P$ 上两点构成的直线,从 $\vec{p_{1}}$ 连接到 $\vec{p}$: $\vec{p} - \vec{p_{1}} = \left(x - x_{1}, y - y_{1}, z - z_{1}\right)$,

[[../../../files/planeGraph01.png]]

其中 $\vec{p_{1}}$ 是一个已知的点, $\vec{p}$ 是任意一个点.用等式表示就是: $\vec{n} \cdot \left(\vec{p} - \vec{p_{1}}\right) = 0$,这个就是平面方程 (plane equation),可以表示平面 $P$.
...
最后会看到的空间形状形成一个几何体(下面会有图片),这个步骤除了裁剪外,还会计算顶点投影后的坐标,所以这个过程也叫做投影变换(projection transformation).
...
这个"容器"是一个立方体,使用的是左手坐标系,三轴的范围分别都是 $\left[-1, 1\right]$.(下面会有图),除此以外还需要把从上一步得到的齐次坐标转化成 3D 坐标,这所有的工作其实只需要一步即可:

$\left(x_{clip}, y_{clip}, z_{clip}, w_{clip}\right)$ 变成正常 3D 坐标 =NDC=: $\left(x_{ndc}, y_{ndc}, z_{ndc}\right) = \left(\frac{x_{clip}}{w_{clip}}, \frac{y_{clip}}{w_{clip}}, \frac{z_{clip}}{w_{clip}}\right)$.
