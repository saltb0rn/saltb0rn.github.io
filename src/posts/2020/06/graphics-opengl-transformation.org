...
最后再提醒一下, 在固定管线的时期, =OpenGL= 的 $M_{modelview}$ 是不能分开成 $M_{model}$ 和 $M_{view}$ 的, 所以那个时期的法线变换只能是 $(M_{modelview}^{-1})^{T}$, 并且用于计算光照的光源是在视点坐标系上的.

后来可以对 $M_{modelview}$ 进行拆分, 光源以及法线变换可以发生在世界坐标系上了, 这个时候的法线变换为 $(M_{model}^{-1})^{T}$,

如果 $M_{model}$ 只包含了等比缩放和旋转, 并且只考虑法线变换后的方向, 那么可以通过该计算得出法线方向: $M_{model} \left(\begin{array}{c} n_x \\ n_y \\ n_z \\ n_w \end{array} \right)$,
