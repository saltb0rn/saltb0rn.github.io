...
总上所属, 如果相机的模型变换为: $M_{camera} = M_{T}M_{R}$, $M_{T}$ 是相机的平移, $M_{R}$ 是相机的旋转,
...
而 $M_{R}^{-1}$ 就不太好看出来了, 因为仅凭这三个参数不足以解析相机是围绕哪根轴往什么方向旋转了多少度, 所以通过旋转矩阵来构建 $M_{R}^{-1}$ 不现实.

但我们知道视点坐标系是一个左手正交坐标系, 它的 $z$ 轴正方向和右手坐标系上的相反, 并且能通过参数 $eye$ 和 $center$ 计算出左手坐标系上的 $z$ 轴,
...
令 $\hat{r}_{\text{right-hand}} = \left( \begin{array}{c} x_r \\ y_r \\ z_r \end{array} \right)$, $\hat{u}_{\text{right-hand}} = \left( \begin{array}{c} x_u \\ y_u \\ z_u \end{array} \right)$, $\hat{f}_{\text{right-hand}} = \left( \begin{array}{c} x_f \\ y_f \\ z_f \end{array} \right)$, 把它们组合成一个右手坐标系可以得出 $M_{R} = \left( \begin{array}{c} x_r & x_u & - x_f & 0 \\ y_r & y_u & - y_f & 0 \\ z_r & z_u & - z_f & 0 \\ 0 & 0 & 0 & 1 \end{array} \right)$.

由于 $M_{R}$ 是一个旋转矩阵, 而旋转矩阵是[[https://en.wikipedia.org/wiki/Orthogonal_matrix][正交矩阵]], 正交矩阵的逆矩阵就是它的转置矩阵, 所以 $M_{R} = \left( \begin{array}{c} x_r & y_r & z_r & 0 \\ x_u & y_u & z_u & 0 \\ - x_f & - y_f & - z_f & 0 \\ 0 & 0 & 0 & 1 \end{array} \right)$.

#+begin_quote
事实上, 这只是其中一种构建方式, 主要区别在于 $forward$ 的计算, 它们的是 $forward = eye - center$.

这导致了后面对于 $\hat{r}_{\text{right-hand}}$ 以及 $\hat{u}_{\text{right-hand}}$ 的计算也发生了改变:

$\begin{cases} right = up \times \hat{f}_{\text{right-hand}} \rightarrow \hat{r}_{\text{right-hand}} \\ \hat{u}_{\text{right-hand}} = \hat{f}_{\text{right-hand}} \times \hat{r}_{\text{right-hand}} \end{cases}$

当然, 最后的矩阵是一样的. 另外, 这里参考了以下内容:
