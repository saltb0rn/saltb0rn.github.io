...
***** 球面坐标 (Spherical Coordinates)
      ...
      球面坐标 $(\rho, lat, long)$ 使用三个参数描述一个点的位置的,

      这三个参数的含义是: $\rho$ 表示原点到 $p$ 点的距离,也就是球体的半径 $r$, $lat$ 和 $long$ 依然分别是纬度和经度.
      ...
      可以得知在以 $O_{sphere}$ 为原点的坐标系下, $p$ 的坐标就是 $(atan2(x, z) r, \frac{\pi}{2} r - acos(\frac{y}{r}) r)$,

      平移原点后 $p = (\pi r + atan2(x, z)r, \frac{\pi}{2} r - acos(\frac{y}{r}) r + \frac{\pi}{2} r) = (\pi r + atan2(x, z) r, \pi r - acos(\frac{y}{r}) r)$;

      然后每个坐标都映射到 $\left[0, 1\right]$ 上得到 $(\frac{\pi r + atan2(x, z) r}{2 \pi r}, \frac{\pi r - acos(\frac{y}{r}) r}{\pi r})$,

      最后得到结果 $\phi(x, y, z) = (\frac{\pi + atan2(x, z)}{2 \pi}, \frac{\pi - acos(\frac{y}{r})}{\pi})$, 其中 $r = \sqrt{x^{2} + y^{2} + z^{2}}$.
...
***** 柱坐标系 (Cylindrical Coordinates)
      ...
      来看一下点 $p = (x, y, z)$ 和柱坐标的关系,首先$p$ 只会在柱体顶部和底部以外的表面上,

      $\rho$ (注意,这不是字母 $p$) 是柱体的半径,等于 $\sqrt{x^{2} + z^{2}}$, $\theta$ 是点 $p$ 在上图的夹角, $y$ 就是 $p$ 的 $y$ 坐标.
      ...
      在以 $O_{cylinder}$ 为原心的坐标下, $p$ 的坐标就是 $(atan2(x, z)\rho, y)$,

      # 平移原点 $(\pi\rho + atan2(x, z)\rho, \frac{h}{2} + y)$,

      # 映射到 $[0, 1]$ 上 $(\frac{\pi\rho + atan2(x, z)\rho}{2\pi\rho}, \frac{\frac{h}{2} + y}{h}) \rightarrow (\frac{\pi + atan2(x, z)}{2\pi}, \frac{1}{2} + \frac{y}{h})$.

      在经过原点平移和映射到 $[0, 1]$ 后得到 $(\frac{\pi + atan2(x, z)}{2\pi}, \frac{1+y}{2})$.

      *(不太清楚这个v是如何推导得到的,[[https://gamedev.stackexchange.com/questions/114412/how-to-get-uv-coordinates-for-sphere-cylindrical-projection][这里]]给出了解释,但本人还是不懂这么推导)*
