...
全称 =Signed Distance Function= 或 =Signed Distance Field=, 这里用函数 $f$ 表示.
...
其实在 [[#smoothstep-draw][The Book Of Shaders 的 smoothstep 函数绘制 Shader 解析]] 就已经运用到中心差商法了, 只不过并没有直接把导数算出来而已.
...
$\begin{cases} \vec{RP} = (x_p, y_p) \\ \vec{RQ} = (x_q, y_q) \\ \vec{RP} \cdot \vec{RQ} = x_p x_q + y_p y_q \\ \cos \alpha = \frac{\vec{RP} \cdot \vec{RQ}}{|\vec{RP}| |\vec{RQ}|} \end{cases}$
...
可以发现 $|\vec{QP} \times \vec{RP}| = |\vec{PQ} \times \vec{RQ}| = |x_{p}y_{q} - x_{q}y_{p}|$.
...
$\begin{equation*} \begin{aligned} & t^2 (\vec{PQ} \cdot \vec{PQ}) + 2t (\vec{PQ} \cdot \vec{RP}) + \vec{RP} \cdot \vec{RP} \\ = & t^2 [(Q - P) \cdot (Q - P)] + 2t [(Q - P) \cdot (P - R)] + [(P - R) \cdot (P - R)] \\ = & t^2 |Q - P|^2 + 2t [(Q - P) \cdot P] + |P|^2 \end{aligned} \end{equation*}$
...
$\begin{equation*} \begin{aligned} & \int_{0}^{1} \frac{y^{'}(t) x(t) - y(t)x^{'}(t)}{x(t)^2 + y(t)^2} dt \\ = & \int_{0}^{1} \frac{D}{t^2 \langle Q - P | Q - P \rangle + 2t \langle Q - P | P \rangle + \langle P | P \rangle} dt \\ = & D \int_{0}^{1} \frac{1}{t^2 \langle Q - P | Q - P \rangle + 2t \langle Q - P | P \rangle + \langle P | P \rangle} dt \end{aligned} \end{equation*}$
...
因此, 想要求出 $t^2 \langle Q - P | Q - P \rangle + 2t \langle Q - P | P \rangle + \langle P | P \rangle$ 的反导数就得把它转换成平方和的形式: $u^2 + v^2$.

这里令 $\begin{cases} a = \langle Q - P | Q - P \rangle \\ b = \langle Q - P | P \rangle \\ c = \langle P | P \rangle \end{cases}$, 令 $f(t) = at^2 + 2bt + c = a(t^2 + \frac{2b}{a} t) + c$,
...
$\begin{equation*} \begin{aligned} & D \int_{0}^{1} \frac{1}{t^2 \langle Q - P | Q - P \rangle + 2t \langle Q - P | P \rangle + \langle P | P \rangle} dt \\ = & \left. D (\frac{1}{\sqrt{ac - b^2}} \tan^{-1}(\frac{at + b}{\sqrt{ac - b^2}}) + C) \right]_0^1 \\ = & \left. D (\frac{1}{D} \tan^{-1}(\frac{at + b}{D}) + C) \right]_0^1 \\ = & \tan^{-1}(\frac{a + b}{D}) - \tan^{-1}(\frac{b}{D}) \end{aligned} \end{equation*}$

其中 $a + b = \langle Q - P | Q - P \rangle + \langle Q - P | P \rangle = \langle Q - P | (Q - P + P) \rangle = \langle Q - P | Q \rangle$,

$\begin{equation*}\begin{aligned} & \tan^{-1} \frac{a + b}{D} - \tan^{-1} \frac{b}{D} \\ = & \tan^{-1} \frac{\langle Q - P|Q \rangle}{D} - \tan^{-1} \frac{\langle Q - P | P \rangle}{D} \\ = & \tan^{-1} \frac{\langle Q - P|Q \rangle}{D} + \tan^{-1} \frac{\langle P - Q | P \rangle}{D} \\ = & \mathrm{sign}(D) (\tan^{-1}\cot \gamma + \tan^{-1}\cot \beta) \end{aligned} \end{equation*}$
...
#+BEGIN_SRC glsl
  ...
  float sdfPoly(vec2 p, vec2[N] vs) {
    ...
    for (int i = 0, j = num - 1; i < num; j=i, i++) {
      ...
      /*
        ...
        如果还想要对环绕数进行变化, 那么这时候只能让 ab \times ap > 0, 因为边往上走;
  ...
  void main () {
    ...
    vec2 v0 = 0.8*cos( 0.40 * u_time + vec2(0.0,2.00) + 0.0 );
    vec2 v1 = 0.8*cos( 0.45 * u_time + vec2(0.0,1.50) + 1.0 );
    vec2 v2 = 0.8*cos( 0.50 * u_time + vec2(0.0,3.00) + 2.0 );
    vec2 v3 = 0.8*cos( 0.55 * u_time + vec2(0.0,2.00) + 4.0 );
    vec2 v4 = 0.8*cos( 0.60 * u_time + vec2(0.0,1.00) + 5.0 );
    ...
    // float l = smoothstep(0.0, 1.0 / u_resolution.x, abs(d));
    // gl_FragColor = vec4(vec3(l), 1.0);
  }
#+END_SRC
...
# https://blog.csdn.net/qw8704149/article/details/121867120?spm=1001.2101.3001.6661.1&utm_medium=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ECtr-1-121867120-blog-105440620.235%5Ev43%5Epc_blog_bottom_relevance_base9&depth_1-utm_source=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ECtr-1-121867120-blog-105440620.235%5Ev43%5Epc_blog_bottom_relevance_base9&utm_relevant_index=1
