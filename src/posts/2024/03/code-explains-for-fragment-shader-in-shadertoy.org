...
本文会在一些地方尽量说明引用了什么数学定理, 方便读者(包括我本人)遇到不会的数学知识可以自行去搜索学习.
...
不过 =Inigo Quilez= 的文章要求读者有前面提到的数学基础, 所以读不懂不要气馁, 等掌握了基础再来读即可.

另外, 有很多时候需要作图以及计算等式, 所以个人建议最后学一两个这方面的工具, 简单作图的话推荐 [[https://graphtoy.com][GraphToy]], 想要作一些复杂的图以及公式计算则推荐 [[https://maxima.sourceforge.io/][Maxima]].

个人认为学数学应 *尽可能* 地对给出定理刨根问底, 具体做法是:
...
最后就是对于知识的运用: 要知道哪些知识可以解决什么问题, 想要运用自如就需要经常做题练习, 等到运用自如这一步, 看问题就很容易联想到这些学过的知识了, 这就是俗称的灵感,

正如马斯洛锤子定理: 拿着锤子的人看什么都像钉子. 拿着各种工具看到的景象就不一样了.
...
接着是阴影的形成, 原理类似, 从光源 $L$ 发出数条阴影射线, 当视觉射线与物体的最近相交点为 $S$, 便以 $L - S = \vec{SL}$ 作为其中一条阴影射线的方向, 再找出这条阴影射线上离光源最近的交点 $J$, 如果 $|J - L| \lt |S - L|$, 那么 $S$ 就处于阴影之中, 在视觉射线所对应的像素上设置阴影色; 否则 $|J - L| = |S - L| \rightarrow J = S$, 点 $S$ 不处于阴影之中, 无需为对应像素设置阴影色.
...
假设要判断位于 $p_0$ 的相机在方向 $\vec{rd}$ (是单位向量) 上发射出的一条视角射线是否与物体相交, 先在 $p_0$ 上找到距离步长 $d_1$ 并前进到 $p_1$, 只有这个步长可以保证所有从 $p_1$ 出发的任意方向射线不会进入物体内部, 比如步长 $e_1$ 会让某些方向的射线进入物体内, $d_1$ 的特点是所有可选步长中最短, 后续 $d_2$, $d_3$ 和 $d_4$ 都是这么来的. 前进后的坐标计算为 $p_n = p_0 + \sum\limits_{i=0}^{n} d_i \cdot \vec{rd}$, 其中 $d_0 = 0$.

由于光线在前进时不断形成球体检查交点, 所以光线步进也被称为球体追踪(=sphere tracing=).

因为 $\sum\limits_{i=0}^{n} d_i$ 是相机与场景交点的正交距离, 所以可以使用光线步进来计算 =3D= 场景的 =SDF=: 相机到场景的正交距离场.
...
[[central-diff-application][法线向量估算]]
