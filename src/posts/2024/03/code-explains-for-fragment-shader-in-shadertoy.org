...
如果从正面($Z$ 轴方向)进行投影, 以顶点坐标的 $xy$ 分量计算 $uv$.
...
因此, 人们想到了一种比 =3D= 贴图更低消耗的映射方法: 三平面映射(=Triplanar Mapping=)这种算法, 它是平面映射的高阶版本:
...
#+begin_quote
需要注意, 文档里面的公式有一些错误, 这里会对公式进行一些调整.
#+end_quote
...
在左图中, $Q$ 在场景中的 =SDF= ($\mathrm{sdfScene}(P + d \cdot N)$) 远远大于 0;

在右图中, $Q$ 在场景中的 =SDF= 非常接近 0.

换而言之, 在 $Q$ 沿着交点 $P$ 的法线方向 $N$ 移动距离 $d$ 时,

如果 $Q$ 的 =SDF= 接近于 0 且 $d$ 非常小, 那么可以断言 $P$ 被遮蔽了.

在拥有判断是否遮蔽的方法后, 就得思考如何确定遮蔽的程度.

再仔细思考可以发现以下规律:

在没有遮蔽️物的情况下, $Q$ 的 =SDF= 就等于 $d$: $d - \mathrm{sdfScene}(P + d \ cdot N) = 0$.

在有遮蔽物的情况下, $Q$ 的 =SDF= 就小于 $d$: $d - \mathrm{sdfScene}(P + d \cdot N) \gt 0$.
