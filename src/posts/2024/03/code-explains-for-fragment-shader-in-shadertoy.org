...
这里就是专门介绍这些常用的积木, 它们基本上是数学知识, 主要是线性代数, 微积分以及概率论的这三个方面, 当然数值分析也很重要.
...
最后就是对于知识的运用, 当到了知道哪些知识可以解决什么问题, 以及看到问题可以联想到用哪些知识解决,

就说明到达运用自如的地步了, 而这种程度的联想就是俗称的灵感.

想要到达运用自如的境界需要经常做题练习, 正如马斯洛锤子定理: 拿着锤子的人看什么都像钉子,

当手头上的工具多了, 看到的景象就不一样了. 所以, 学数学依靠的是耐心和理解.
...
虽然在直觉上挺相似的, 但它并非人们所知的微积分中的偏导数, 其定义为:
...
#+BEGIN_SRC glsl
  void main() {
    ...
    vec3 rd = GetRayDirection(uv, cameraPos, vec3(0.0, 3.0, 0.0), 0.5); // 重新设置了 lookAtPos
    float d = RayMarch(cameraPos, rd);
    vec3 col = vec3(0.0);
...
使用 [[https://en.wikipedia.org/wiki/Box_blur][盒状模糊]] 实现抗锯齿.
...
如果从右侧($X$ 轴方向)进行投影, 以顶点的 $yz$ 分量作为 $uv$;

如果从顶部($Y$ 轴方向)进行投影, 以顶点坐标的 $xz$ 分量作为 $uv$;

如果从正面($Z$ 轴方向)进行投影, 以顶点的 $xy$ 分量计算 $uv$.

这种映射算法适用于只展示几何体单面的情况, 几何体的其它面会有严重的拉伸和形变.

教材上没有提到三平面映射(=Triplanar Mapping=)这种算法, 它是平面映射的高阶版本:
...
#+BEGIN_SRC glsl
  ...
  Intersection sdfScene(vec3 p) {
    vec3 sp = vec3(0.6, 0.6, 0.6) * p;
    Intersection sphere = Intersection(sdfSphere(sp - sphereCenter, sphereRadius), 1);
  ...
  void main() {
    ...
    for (int aaY = -1; aaY <= 1; aaY++) {
      for (int aaX = -1; aaX <= 1; aaX++) {
        ...
        vec2 uv = (2.0 * coord - iResolution.xy) / min(iResolution.x, iResolution.y);
        vec3 rd = GetRayDirection(uv, cameraPos, vec3(0.0, 0.0, 1.0), 0.5);
        // 调整11: 修改返回值类型
...
但还是要再提醒一点, 在我们的渲染器中, 射线上点的 $p$ 是视点空间上的坐标, 基本上一切坐标都是视点坐标, 包括相机位置, 并且是左手坐标系.
