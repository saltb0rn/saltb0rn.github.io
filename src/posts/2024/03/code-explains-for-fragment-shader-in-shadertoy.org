...
# 添加使用迭代获得邻接片元的颜色例子
# 如果只是想获取其它片元的颜色, 可以按照以下思路.
#+begin_src c
  vec2 uv = UV;
  vec3 pixels[9];
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
      // shift [row, col] from [0, 2] to [-1, 1]
      vec2 uv = uv + vec2(
                          float(col - 1) * emboss_width * 0.01,
                          float(row - 1) * emboss_height * 0.01);

      /* 计算当前片元的相邻片元的 uv
      uv + vec2(-1 * 0.15 * 0.01) | uv + vec2(0, -1 * 0.15 * 0.01) | uv + vec2(1 * 0.15 * 0.01, -1 * 0.15 * 0.01)
   uv + vec2(-1 * 0.15 * 0.01, 0) | uv + vec2(0) = uv              | uv + vec2(1 * 0.15 * 0.01, 0)
   uv + vec2(-1 * 0.15 * 0.01, 1 * 0.15 * 0.01) | uv + vec2(0, 1 * 0.15 * 0.01) | uv + vec2(1 * 0.15 * 0.01)
      ,*/

      // 事实上, 片元之差是 TEXTURE_PIXEL_SIZE, 并非 (emboss_width * 0.01, emboss_height * 0.01)
      // vec2 uv = uv + vec2(
      //                     float(col - 1) * emboss_width * TEXTURE_PIXEL_SIZE.x,
      //                     float(row - 1) * emboss_height * TEXTURE_PIXEL_SIZE.y);
      pixels[row * 3 + col] = texture(TEXTURE, uv).rgb;
    }
  }
#+end_src
...
https://forums.unrealengine.com/t/q-what-is-does-the-rcp-function-perform-in-the-shader-code/28068/2
