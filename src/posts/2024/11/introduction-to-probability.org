...
不考虑抽取顺序的情况下, 总共有 $C_{n+k-1}^{k} = \frac{A_{n+k-1}^{k}}{k!} = \frac{(n + k - 1)(n + k - 2) \dots n}{k!} = \frac{(n + k - 1)!}{(n - 1)!k!}$ 种可能(=P18, Example 1.4.22= 有证明).
...
- P104, *随机变量 (Random Variable)*: 假设有一个样本空间为 $S$ 的实验, 随机变量 $\text{r.v.}$ 是一个将样本空间 $S$ 映射到实数 $\mathbb{R}$ 的函数.
  ...
  在通过实验获得样本空间后, 我们需要选定实验结果的 *特征* 给实验结果进行分类, 并且使用数字给每个分类进行编码, 每个分类的数字都不一样.

  简单而言, 随机变量的核心作用是把非数值的随机事件结果转化为数值形式, 以便应用数学工具进行概率分析.

  通过定义一个随机变量 $X$ 来量化实验结果的某个特征, 部分实验结果 $s$ 会在该特征上呈现一致, 因此,

  把这些实验结果划分到一个分类 $E$ 中, $s \in E$, 一个分类就是一个事件,

  所以有时候会看到这样的描述 "事件 $E$ 的随机变量 $X$ 为多少", 其实就是说 $X(s)$ 的值是多少.

  比如随机抽取 =100= 个人, 按照身高范围对实验对象进行分类, 那么用随机变量 $X$ 表示实验对象的身高等级,

  把身高在 $170\text{cm}$ 到 $180\text{cm}$ 的实验对象划分到分类 $E$ 中, 用 $4$ 作为它的等级, 也就是当 $s \in E$ 时, $X(s) = 4$.

  编码方案并不是固定的, 可以把 $4$ 换成其它数字, 取决于个人喜好, 但是在一个编码方案下的映射关系是固定的.
...
- P106, *离散型随机变量 (Discrete random variable)*
  ...
  所谓 *可数无限* 是指可以数但数不完, 是无穷大的一种表现. 自然数集, 整数集和有理数集都是可数无限的,

  通俗点说就是 *离散* 无限集合, 而不可数无限是 *连续* 无限集合的表现, 比如实数集就是不可数无限.
  #+end_quote
...
- P172, 方差的属性
  ...
  - $\text{Var(X)} \le 0 \Leftrightarrow P(X = a) = 1$, 其中 $a$ 是某些常数.
    ...
    当 $P(X = a) = 1$ 时, $E(X) = a \dot P(X = a) + 0 = a$.
...
- P273 & P275, *偏度/偏态 (Skewness)*: 设随机变量 $X$ 的平均值为 $\mu$, 其方差为 $\sigma^2$, 它的偏度是它的第 $3$ 标准矩: $\text{Skew}(X) = E((\frac{X - \mu}{\sigma})^3)$.
  ...
  当偏度为 0, 分布关于它的期望对称; 当偏度为正数, 平均值大于中位数, 中位数大于众数, 分布的图像显得左厚右薄;

  当偏度为负数, 平均值小于中位数, 中位数小于众数, 分布的图像显得左薄右后.
...
- P279, *矩生成函数 (Moment generating function, MGF)*: 一个随机变量 $X$ 的 =MGF= 为 $M(t) = E(e^{tX})$, 其中 $M$ 在定义域 $(-a, a)$ 上有效且里定义域包含 0, 否则 $X$ 的 =MGF= 不存在.
...
- P326, *协方差 (Covariance)*
  ...
  = & E(XY) - E(X(E(Y))) - E(Y(E(X))) + E(E(X)E(Y)) && \text{出现在期望运算符} E \text{中的期望} E(X) \text{和} E(Y) \text{可以看作是常数} \\
...
- P329, *相关性 (correlation)*: 随机变量 $X$ 和 $Y$ 之间的相关性为 $\text{Corr}(X, Y) = \frac{\text{Cov}(X, Y)}{\sqrt{\text{Var}(X) \text{Var}(Y)}}$.
  ...
  如果 $\text{Corr} = 0$, 那么一个变量不会随着另外一个变量线性地增减, 即不存在线性相关性, 且可能存在非线性关系.

  # $Y = f(X)$, 当 $\text{Corr} = 0$ 时, $f$ 是非线性函数, 否则 $f$ 是线性函数.

  另外, 对 $X$ 和 $Y$ 进行平移和缩放 *不会* 对 $X$ 和 $Y$ 之间的关联性产生影响.

  独立性和相关性差别在于:

  独立性衡量的是两个变量之间是否存在任何形式的统计依赖(概率关联), 即一个变量的分布是否受到另一个变量取值的影响;

  相关性衡量的是两个变量的变化是否存在统计上的线性关系. 当两者独立时, 相关性必然为 0, 不存在线性或非线性关系;

  但反过来不成立, 相关性为 0 只能确定两者不存在线性关系, 可能存在非线性关系, 因此不能说明两者独立.

  相关性不能说明因果关系: 即 $A$ 导致 $B$. 而是说明存在共同原因 $C$ 同时导致 $A$ 和 $B$.

  这里不能搞混 "不独立" 与 "存在因果关系" 两者, 独立性是特指概率关联是否存在的, 因果关系是一个变量对另一个变量的作用.
...
比如模拟圆周率的值, 可以取一个 $2 \times 2$ 的正方形, 里面嵌入一个半径为 1 的圆, 即圆正好内接于正方形.

它们的面积分别为 $4$ 和 $\pi$, 随机往这个正方形区域抛硬币 $S$ 次,

统计落在圆内的次数为 $C$, 圆与正方形之间的面积比为: $\frac{C}{S} = \frac{\pi}{4}$,

利用该概率可以得出圆形的面积为 $\pi \approx \frac{4 C}{S}$, 而圆的面积则是作为积分结果.
...
这样就可以需要少量的采样数来达到较为精确的结果. 但这样的采样就是有偏向的了,

因此这种采样法被称为重要性采样(=Importance Sampling=).
...
int main() {
    ...
    exact = 1 - cos(1); // f 在 [a=0, b=1] 上的精确积分值为: -cos(b) - -cos(a) = -cos(1) + cos(0)
...
$\begin{equation*} \begin{aligned} & \int_{a_d}^{b_d} \dots \int_{a_2}^{b_2}\int_{a_1}^{b_1} f(x_1, x_2, \dots, x_d) dx_1 dx_2 \dots dx_d \\ & = \int_{a_d}^{b_d} \dots \int_{a_2}^{b_2}\int_{a_1}^{b_1} \frac{f(x_1, x_2, \dots, x_d)}{p(x_1, x_2, \dots, x_d)} p(x_1, x_2, \dots, x_d) dx_1 dx_2 \dots dx_d \\ & = E\left[\frac{f(x_1, x_2, \dots, x_d)}{p(x_1, x_2, \dots, x_d)}\right] \\ & \approx \frac{1}{n} \sum \limits_{i=1}^{n} \frac{f((x_{1})_i, (x_{2})_i, \dots, (x_{d})_i)}{ p((x_{1})_i, (x_{2})_i, \dots, (x_{d})_i)} \end{aligned} \end{equation*}$

为了方便学习, 这里假设 $p$ 是随机变量之间独立的高维均匀分布概率密度函数: $p(x_1, x_2, \dots, x_d) = \frac{1}{\left[ \prod_{j=1}^d (b_j - a_j) \right]}$, 那么:

$\int_{a_d}^{b_d} \dots \int_{a_2}^{b_2}\int_{a_1}^{b_1} f(x_1, x_2, \dots, x_d) dx_1 dx_2 \dots dx_d \approx \left[ \prod_{j=1}^d (b_j - a_j) \right] \frac{1}{n} \sum\limits_{i=1}^{n} f((x_{1})_i, (x_{2})_i, \dots, (x_{d})_i)$.
