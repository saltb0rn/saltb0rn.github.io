...
window.adoc umtListener('resize', function(event){
    html.style.fontSize = document.documentElement.clientWidth / 10 + 'px';
});
#+END_SRC
...
原因是要兼容 =vw= 单位.使用该方案可以免去通过 =JS= 动态设定根接点的字体大小以及构建工具,除此以外和 =rem= 方案的用法以及计算方法都是一样的.具体实现就不说了,

就是把上面的 =100rem= 或者 =10rem= 换成 =100vw=.
