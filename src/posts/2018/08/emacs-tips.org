...
2. =Hash= 这个集合(计算出名字索引,也就是名字的 =hash code=),并且在一个叫做 =obarray= 的表中查找索引.
   ...
   =obarray= 本质就是一个 =vector=,它的每一个元素就是一个桶(bucket),每个桶储存了一个 =0= 或一个 =interned symbol=, =0= 表示桶为空,

   (*这里开始文档的说法有点绕,我的塑料英语这里可能会理解有误*)对于 =symbol=,先计算出 =symbol= 的名字的 =hash code= (是 =vector= 的下标),再根据 =hash code= 把 =symbol= 的名字存放到在对应的桶里面(参考 [[https://research.cs.vt.edu/AVresearch/hashing/strings.php][Hash Functions for Strings]] 和 [[http://www.cse.yorku.ca/~oz/hash.html][Hash Functions]]).
...
该{{{buffer}}}就是进程关联的{{{bufer}}}.如果进程没有关联{{{buffer}}},那么默认的过滤函数就会丢弃输出.
