...
** 使用Emacs的一些tips
   ...
   - 快速做正则测试 (一次关于是否使用 =\= 的思考,不仅仅限于 =Emacs Lisp=)
     ...
     具体做法就是使用 =\= 字符放在需要进行选择的字符前面, 如果被标记的字符是有正则定义的特殊字符,
     ...
     你可以理解为 =minibuffer= 里面写的并不是字符串(, 毕竟没有双引号包着), 而是转化成字符串之前的形态,

     最终会在 =Emacs= 的内部处理中转化成字符串; 而写代码的时候就是内部处理了.
     #+END_QUOTE
     ...
     此外, =Emacs Lisp= 的正则也像字符串那样定义了一些有着特殊含义的字符序列, 他们被称为特殊构造(special constructs).
