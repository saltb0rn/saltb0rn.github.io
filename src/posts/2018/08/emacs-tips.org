...
*** Emacs 编码系统
    ...
    *编码标准* 就是一套给字符集编号的方案,下面就举个不恰当例子来 *直观地* 了解一下这个过程,
    ...
    没有规定 1 个字节就是 8 位的过去给计算机的发展带来这很多问题(这里有[[https://softwareengineering.stackexchange.com/questions/120126/what-is-the-history-of-why-bytes-are-eight-bits][这方面的讨论]]),为了避免这些问题,人们默契地统一 8 位(bits)为 1 个字节(byte), *Unicode Standard* 也不例外.
    ...
    *UTF-8* 表示 1 个字节作为基本编码单位,因为每个字符需要的空间不是一样的,因此需要一些位来标识是多少个字节为一个字符,
    ...
    这样最高就需要用到 4 个字节了,很少用到到的字符都放到最后面了,所以大部分情况都是 1 个字符需要 1 到 3 个字节这样子.

    比如"盐"字的 =codepoint= 为 30416,十六进制是 =#x76D0=,二进制是 =#b0111 0110 1101 0000=,

    处于第三个区间,那么按照这个顺序 =0111 011011 010000= 填入到对应区间的模板最后得到的 =11100111 10011011 10010000= (E79B90) 就是"盐"字的 *UTF-8* 编码.

    *UTF-16* 是指最少要用 2 个字节作为基本编码单位,也就是在这种编码下的字符需要 2n 个字节;

    *UTF-32* 是指最少要用 4 个字节作为基本编码单位,也就是在这种编码下的字符需要 4n 个字节,n 是整数.

    *UTF-16* 和 *UTF-32* 都以多字节作为基本编码单位,这就需要讨论一下多字节编码的排序问题了,

    我们目前接触到的都是字节从左到右,从大位/高位到小位/低位的顺序排列的,但是在计算机里面并不一样是这样的,

    对于每个需要 *多字节编码* 的字符,它们的字节排列顺序可能有两种: 大端(*big endian*)和小端(*little endian*), =endian= 是 =end= 的变形.

    这其实不是文本文件特有的问题,所谓的大端就是按照从低位往高位方向 *增长* 的顺序排列数据的 *基本单位*,也就是从高位到低位顺序排列,一般往左边就是高位方向,

    就是从左到右排序,比如 10 进制数字 1230, 1 是千位, 2 是百位, 3 是十位, 0 是个位,这里的基本单位就是数位,这种排列顺序符合大端的定义;

    小端则是相反,从高位往低位方向 *增长* 排列数据的基本单位, 1230 的小端排列就是 0321.

    但计算机可没有这么简单,这是因为基本单位的选取是很复杂的, *UTF-16* 和 *UTF-32* 的情况相对复杂,

    为了讲明白这大小端是什么概念,先抛开文本的编码以及解码从一个简单的例子说起:

    现在需要把 1230 储存到计算机里面,我们假设计算机编码数字的原理是把数字的二进制形式和数字本身等同起来,并且规定一个字节为一个基本单位,

    1230 的二进制是 =#b10011001110=,由于二进制对于人类而言读写都不方便,因此后面用它的 16 进制 =#x4ce= 来表示它的二进制,

    按照大端定义,1230 在计算机里面的样子是这样的 =04 ce= 一个由两个字节组成的字节序列(补一个0是因为计算机现在规定了编码一个数据最少要为一个字节),
    ...
    所以,判断数据是使用大端还是小端编码对于字节流分析来说是很重要的,如果我总结得不清楚的话可以看 [[https://www.digital-detective.net/understanding-big-and-little-endian-byte-order/][understanding big and little endian byte order]].

    *UTF-16* 和 *UTF-32* 都分大端和小端两种编码,比如就 *UTF-16* 来说,它一个数据(也就是一个字符)需要 2n 个字节,基本单位还是一个字节,
    ...
    *UTF-16* 和 *UTF-32* 可能会在文件的开头采用 =BOM(Byte Order Mark)= 来声明自己采用哪种顺序,有没有还是要看具体程序如何编码,
    ...
    而 *UTF-8* 是不分大小端的,它只有大端,所以 =BOM= 对于 *UTF-8* 编码的文件是没有意义的,然而很多 =Windows= 程序会在 *UTF-8* 文件前面加上无意义的 =BOM=,这是微软开的坏头,
