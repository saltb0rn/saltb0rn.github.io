...
** 使用Emacs的一些tips
   ...
   - 快速做正则测试 (一次关于是否使用 =\= 的思考,不仅仅限于 =Emacs Lisp=)
     ...
     首先要清楚一点, =Emacs Lisp= 严格上来说没有正则表达式这个数据类型, 它是直接用字符串来表示正则表达式.

     这么说你可能不太理解, 就比如 =Python= 和 =JavaScript=, 它们都有单独的语法来表示正则表达式.
     ...
     我们把表示字符本身的叫做表达字符含义, 发挥正则定义叫做表达正则含义.

     由此, 一些字符的含义发生了冲突, =Emacs Lisp= 要求开发人员要对字符的含义进行选择.

     具体做法就是使用 =\= 字符放在需要进行选择的字符前面, 如果被标记的字符是由正则定义的特殊字符,

     那它就会方向表达为字符含义, 否则就直接表达为正则含义.
     ...
     但是如果你是在写代码的时候进行匹配, 那么就得这么写:

     #+BEGIN_SRC elisp
     (re-search-forward "\\.")
     #+END_SRC

     你可以理解为 =minibuffer= 里面写的并不是字符串, 而是转化成字符串之前的形态.
     #+END_QUOTE

     虽然说看上去对没有正则含义的字符使用 =\= 标记好像什么问题, 但字符串本身也是有转义字符(escaped characters)的,

     这种字符就是以 =\= 开头的字符序列, 比如换行符 =\n=, 因此开发人员不能乱用 =\= 对字符进行标记.

     此外, =Emacs Lisp= 的正则也像字符串那样定义了一些有着特殊含义的字符序列, 他们被称为特别构造(special constructs).
