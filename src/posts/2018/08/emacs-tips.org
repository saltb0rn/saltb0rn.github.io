...
*** Emacs 编码系统
    ...
    *编码标准* 就是一套给字符集编号的方案,下面就举个不恰当例子来 *直观地* 了解一下这个过程是怎么样的吧,
    ...
    由于这个字符集实在是太大了,如果像上面的例子那样直接按照字符集元素个数来分配每个字符的空间,那么每个字符需要 =#x10FFFF= 位空间,

    所以编码标准需要一些优化,比如英文以及基础标点符号只有 256 个字符,那么这种字符就用一个字节空间储存,中文字符需要更多,高达两个字节,其它字符也占用对应空间.
    ...
    =Emacs= 做为文本编辑器,是不可能避开编码问题的,而如果要编写 =Emacs Lisp= 处理文本的话,那么 =Emacs= 内部文本机制是基础了.
    ...
    因为并不是所有文件都是用的 *Unicode Standard* 储存的,而 =Emacs= 做为一款灵活的文本编辑器可不能只支持一种编码标准的文件,

    每次 =Emacs= 打开文件都需要先把文件内容写入到{{{buffer}}}中,这时候的文件内容叫做 =encoded text=,它不是真的 =text=,在 =Emacs= 眼里是一个由多个独立字节排列成的序列,

    然后 =Emacs= 根据文件储存时的编码标准把 =encoded text= 转换为 =Emacs= 编码标准能够解析的结构(另外一个种形式的字节序列),最后才显示为平时看到的文本,
