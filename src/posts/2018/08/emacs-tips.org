...
*** 宏 (Macros)
    ...
    而宏不是, *宏是一种能够生成代码的功能*, 调用宏会先根据定义生成代码,生成的代码 *在之后* 会被直译器执行得到结果.

    生成代码的这个过程叫做展开(expanding),生成的代码叫做展开式(expansion).
...
*** 线程 (Threads)
    ...
    为了让计算机 *看上去* 能够像忍者那样(通过高速移动)分身同时处理多个任务,人们想到了让 =CPU= 在一段时间内执行任务 =A=,

    这个时间是非常短的,时间结束后记录下任务 =A= 的进度,然后 =CPU= 去执行任务 =B=,执行一段时间后同样记录下任务 =B= 的进度,

    接着 =CPU= 去执行任务 =C= ,..., 如此类推,经过一轮切换又回到 =A= 这个未完成的任务上,

    假如 =A= 在上一轮中已经完成了,那么这一轮以及之后的切换都不会切换到 =A= 而是下一个未完成的任务的执行上了;

    如果其中个任务的优先级比较高,不需要等到所有任务都切换一轮地情况下,更频繁地切换到它的执行上,
    ...
    因此同一个进程下的线程们占用同一块内存区域,也就是进程占用的区域,线程之间可以通过访问/修改同一个内存地址实现"交流",

    现在有一个需求: 有两个线程,其中一个线程需要修改某个内存地址,另外一个线程访问这个内存地址来获得修改后的值,

    但是因为是抢占式调度,如果一个线程在修改到一半时被强迫让出 =CPU=,这个时候被修改的内存是没有被释放的,另外的线程获取到的就是修改前的值.

    为此必须要确保线程们的执行顺序能够正确完成任务,就是保证线程同步: 避免两个或者更多的线程同时访问/修改同一个内存地址.
...
*** Emacs 编码系统
    ...
    *编码标准* 就是一套给字符集编号的方案,下面就举个不恰当例子来 *直观地* 了解一下这个过程,
    ...
    其次,键盘有一些比如 =Ctrl=, =Enter= 这样的特殊按键,128 个字符是不够用的.

    人们需要定义了一些足以解决这些问题的字符集, *Unicode Standard* 就是其中一个, *Unicode Standard* 最早也是只支持很小的一部分字符的,

    从用 5 位表示一个字符,到用 6 位表示一个字符,后来再到的用 7 位表示一个字符,这三个编码标准也就是我们今天所知道的 =UTF-5=, =UTF-6= 以及 =UTF-7=.

    =UTF-7= 已经可以支持所有 =ASCII= 字符了,这个时候已经是 1990 年代中期了,

    没有规定 1 个字节就是 8 位的过去给计算机的发展带来这很多问题(这里有[[https://softwareengineering.stackexchange.com/questions/120126/what-is-the-history-of-why-bytes-are-eight-bits][这方面的讨论]]),为了避免这些问题,人们默契地把 8 位(bits)规定为 1 个字节(byte),

    *Unicode Standard* 也开始用 1 个字节作为最小单位来对字符进行编码.

    到了 *Unicode Standard* 的第 4 个版本总共支持 =#x10FFFF= 个字符, 字符码范围是 =0..#x10FFFF=,
    ...
    因此出于对空间节省的考虑,编码标准需要一些优化,比如经拓展后的 =ASCII= 字符集只有 256 个字符,

    那么这种字符就用一个字节空间储存足够了,中文字符需要更多,高达两个字节,如此类推,其它字符也占用对应空间.

    *Unicode Standard* 为此定义了三套编码标准: *UTF-8*, *UTF-16* 以及 *UTF-32*, *UTF-8* 是最常见的一套编码标准,[[https://www.unicode.org/standard/principles.html][这里是官方页面.]]

    涉及到多字节编码就有必要提一下字节的排列顺序了,多字节的编码不一定是按照我们的直觉那样从左到右,从大位/高位到小位/低位的顺序排列字节的,

    对于每个需要 *多字节编码* 的字符,它们的字节排列顺序可能有两种: 大端 (*big endian*) 和 小端 (*little endian*), =endian= 是 =end= 的变形.

    为了讲明白这两个是什么概念,先抛开文本的编码以及解码,举一个不太恰当的例子:

    现在需要把 1230 储存到计算机里面,假设计算机编码数字的原理是把数字的二进制形式和数字本身等同起来,

    1230 的二进制是 =#b10011001110=,由于二进制对于人类而言写都不方便,因此后面用它的 16 进制 =#x4ce= 来表示它的二进制.

    这里把 1230 称为一个数据,由于计算机现在规定了编码一个数据最少要为一个字节,那么 1230 在计算机里面的样子是这样的 =04 ce= 一个由两个字节组成的字节序列.

    所谓的大端就是按照从左到右,从大位到小位的顺序对一个数据的字节进行排列, 那么这里 =04= 是第 1 位, =ce= 是第 0 位,因此 1230 在计算里面的大端表示就是 =04 ce=,

    如果不能理解的话可以看看 1230 的十进制,1 是千位, 2 是百位, 3 是十位, 0 是个位,这是按照大数位到小数位来排序,大端就类似这样符合人类直觉,

    解析回 1230 的过程很简单: $0 \times 16^{3} + 4 \times 16^{2} + c \times 16^{1} + e \times 16^{0} = 1230$.

    小端则是按照从右到左,从大位到小位的顺序对一个数据的字节进行排列,因此 1230 在小端模式下的字节序列是: =ce 04=,

    如果直接解析成 10 进制会出问题: $c \times 16^{3} + e \times 16^{2} + 0 \times 16^{1} + 4 \times 16^{0} = 52740$,

    所以,判断数据是使用大端还是小端编码对于字节流分析来说是很重要的,如果我总结得不清楚的话可以看[[https://www.digital-detective.net/understanding-big-and-little-endian-byte-order/][这里]].

    如果在这基础上增加一个按照解析后的结果 1230 来查找字符的步骤,那么这整个过程就是我们之前数熟悉的文本编码和解码了.

    *UTF-16* 和 *UTF-32* 都分大端和小端两种编码,它们会在文件的开头采用 =BOM(Byte Order Mark)= 来声明自己采用哪种顺序.

    而 *UTF-8* 是不分大小端的,它只有大端,所以 =BOM= 对于 *UTF-8* 编码的文件是没有意义的,

    然而很多 =Windows= 程序会在 *UTF-8* 文件前面加上 =BOM=,这就是微软开的坏头,

    这些文件并不是标准的 *UTF-8* 编码,在其它系统上打开这些文件可能会因为编码引起解析问题.


    =Emacs= 做为文本编辑器,自然是不可能避开编码问题的,而如果要编写 =Emacs Lisp= 处理文本的话,那么理解 =Emacs= 内部文本机制是必要基础了.

    =Emacs= 没有直接使用 *Unicode Standard*,而是在 =Unicode= 的基础上进行拓展,在 =#x110000..#x3FFFFF= 的范围给一些 *无法用 Unicode 和 raw 8-bit bytes 标准解析的字符* 进行编码,
    ...
    这编码之间的转换叫做字符码转换(character code conversion),最后把解析后的内容翻译(解码)成给人类看的字符,这个时候{{{buffer}}}的内容不再是 =encoded text=,
    ...
    也就是把{{{buffer}}}的内容转回 =encoded text=,最后写入到文件中,一般情况是这样的,

    不过 =Emacs= 也支持再保存时选择别的编码标准,这么一来文件就完成了字符码转码了,就像 =Linux= 的 =iconv= 命令一样.
    ...
    通过浏览器把上面请求的页面保存下来,然后通过 =hexl-mode= 使用二进制模式编辑页面,用 *上面的请求内容* 和 *二进制模式下显示的内容* 进行对比后,你会发现:

    前者显示八进制的位置和后者显示 =.= 的位置是对应的,这些位置上的都是 =NON-ASCII= 字符,这就是前面说的不对文件进行转码显示.
