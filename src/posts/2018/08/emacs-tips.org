...
*** Emacs 编码系统
    ...
    *编码标准* 就是一套给字符集编号的方案,下面就举个不恰当例子来 *直观地* 了解一下这个过程是怎么样的吧,
    ...
    总得来说,解决乱码的核心方法就是让 *储存文件时的编码* 和 *读取文件时使用的编码* 保持一致,这两个阶段分别叫做 *encode* 和 *decode*.
    ...
    =Emacs= 作为文本编辑器,和文本打交道自然不可避免的,有一种编码标准叫做 *Unicode Standard*,也叫 *UTF-8*, 它支持多种语言的字符,

    到目前为止(Unicode 4.0)总共支持 =#x10FFFF= 个字符,字符码范围是 =0..#x10FFFF=.

    =Emacs= 没有直接使用这套标准,而是在 =Unicode 4.0= 的基础上进行拓展,在 =#x110000..#x3FFFFF= 的范围给一些 *无法用 Unicode 和 raw 8-bit bytes 标准解析的字符* 进行编码,
    ...
    保存文件则是上面的逆过程: 根据 =Emacs= 编码标准把{{{buffer}}}的内容转换为文件的编码标准所能够解析的结构,也就是转回 =encoded text=,
    ...
    解析前还可以调用 =decode-coding-region= 对当前{{{buffer}}}进行解码,这个例子是最早应该是写在 =Windows= 上的,因为 =Windows= 的 =utf-8= 特有的行尾符号(=^M=)显示出来了,

    这个本不应该显示的,所以这里使用正确的编码标准对{{{buffer}}}重新解码让行尾符号隐藏起来.当然,这个行尾符号不影响 =libxml-parse-html-region= 的解析结果.
