...
*** 变量 (Variables)
    ...
    *在词法作用域下*, =set-a= 里面的 =a= 引用到的是 $a = 1$, 就是引用到的绑定处于在 *文本上* 绑定创建的范围内,
    ...
    这点对于开发模块而言是很糟糕的,这没办法保证模块能够在任何上下文中都能够达到期待的结果.
...
**** 什么是函数 (What is a Function)
     ...
     =Emacs Lisp= 有类函数(=function-like=)的对象,和函数一样都能够执行计算,但并不被认为是函数: =Special Forms 和 Macros=,

     因此在使用 =Emacs Lisp= 进行编程的时候要分清楚以下几个概念:
...
*** 线程 (Threads)
    ...
    请务必理解这种 *CPU逐个执行不同任务的"切片"* 的方式, 这个概念以后会在进程以及异步编程中出现.

    任务的调度方式有两种: 协作式(cooperative or non-preemetive)以及抢占式(preemetive).

    第一种就是在定义任务时设定好计划进度:执行到什么时候为一个阶段,什么时候为下一个阶段,在执行时按照计划进度来执行,

    每当执行完任务的一个阶段时就暂停下来把 =CPU= 让出去执行下一个任务,如此类推,直到所有任务都完成, *任务之间的调度由任务自身决定*,

    不过容易出现一个问题,不能保证所有的任务都能设定好计划进度,如果其中一个任务完全没有设计计划进度,

    那么当 =CPU= 执行到它时就会被一直占领,直到这整个任务完成为止,其它任务就会处于长时间等待 =CPU= 的状态,看起里就是卡住了,

    只是单纯执行时间比较长的话还好,如果这个任务是意外永久占据了 =CPU=,那么其它任务就一直处于等待 =CPU= 的状态,

    第二种则是 *由操作系统对任务进行调度*,操作系统决定任务能够执行多久,一旦时间到了就会强迫当前执行的任务把 =CPU= 让出执行下一个任务,

    这就没有第一种调度方式的弊端了,只是相比第一种调度方式而言开销更大一点,因为任务进度,状态都需要操作系统记录.


    一个线程本质上就是一个任务, =Emacs Lisp= 提供有限的并发机制-线程的支持,它的线程模型基本上是协作式(mostly cooperative, or non-preemetive)线程模型,
