...
*** GNU Emacs Internals
    ...
    可以配合 =gdb= 进行调试,具体参考[[https://wiki.archlinux.org/index.php/Core_dump][这里]],但这不是重点,重点是要学会如何读懂 =core dump=,

    =core dump= 记录了信号(signal),也就是

    比如常见的 =segment fault= 就是 =SIGSEGV=, =Linux= 的[[https://www.man7.org/linux/man-pages/man7/signal.7.html][Singal文档]]就详细.
