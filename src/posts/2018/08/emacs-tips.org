...
*** Emacs 编码系统
    ...
    *编码标准* 就是一套给字符集编号的方案,下面就举个不恰当例子来 *直观地* 了解一下这个过程是怎么样的吧,
    ...
    假设计算机允许显示 =a= 到 =z= 的 26 个字母,计算机给它们每个字符编个号:
    ...
    因为计算机器储存/读取文件都是二进制,这里就用 =<n>= 这种符号来直观表示数字 n 的二进制.

    如果我们在文本看到一个单词: =halo=, 那么文本里面实际储存的数值应该是这样的 =<7><4><11><11>=,
    ...
    计算机按照这套编码标准,每 5 位就翻译得到一个字符,最后就是我们看到 =hell=.

    假设现在有另外一套从 =0= 到 =9= 的字符集,按顺序进行从 0 开始进行编码,因为有 10 种可能的字符,

    那么至少需要 4 位来表示一个字符,如果用这套编码标准打开上面的文件,那么里面的内容就会解析成这样:

    =0011 1000 0001 0110 1110=,这样看到的内容大概就是这样 =3816<14>=,可以发现最后 4 位没有被翻译出来,

    如果上面的文本是 5 个字符的话,用这套编码甚至解析成这样 =0011 1000 0001 0110 1110 xxxx x=,这就是乱码以及乱码发生的原因.

    现实中的计算机不会用 4 位或者 5 位表示一个字符这么 *胡来*,都是用一个字节(8 bits)作为基本单位, =n= 个字节表示一个字符这样的.

    总得来说,解决乱码的核心方法就是让 *储存文件时的编码* 和 *读取文件时使用的编码* 保持一致,使用某种编码标准储存文件/字符叫做编码(*encode*), 以某种编码标准读取文件/字符叫做解码(*decode*).

    当然,同一个文本文件的使用的编码标准并非固定的,可以更选其它编码标准,这就是字符码转换(Character code conversion).

    (PS: 上面的两个例子是没有办法转码,因为彼此没有对方的字符).


    =Emacs= 作为文本编辑器,和文本打交道自然不可避免的.

    最早的时候计算机只支持英文,数字和一些基础的标点符号,后来有别的语言的需求,可以看到上面例子中两门语言的麻烦,

    最好的解决办法就是把上面两门语言的字符集合并在一起,然后给每个字符重新编号.

    现实中就是这样定义了一套支持多种主流语言字符的字符集: *Unicode Standard*, *Unicode 4.0* 总共支持 =#x10FFFF= 个字符,字符码范围是 =0..#x10FFFF=,

    由于这个字符集实在是太大了,如果像上面的例子中那样直接按照字符集的元素个数来给每个字符分配空间的话,那么每个字符需要 =#x10FFFF= 位空间,

    所以编码标准需要一些优化,比如英文以及基础标点符号只有 256 个字符,那么这种字符就用一个字节空间储存,其它字符则占用对应空间.

    为了节省空间, *Unicode Standard* 定义了三套编码标准: *UTF-8*, *UTF-16* 以及 *UTF-32*, *UTF-8* 是最常见的一套编码标准.[[https://www.unicode.org/standard/principles.html][这方面的更多内容就看这里吧.]]
    ...
    所以理论上 =Emacs= 的一个字符需要占用用22位(bit)空间,这就是 Emacs 如何表示一个字符,也就是 =Emacs= 的文本的内部表示(text representation).

    为了节省内存, =Emacs= 并没固定用(fixed-length)22位空间储存一个字符,而是用一个长度可变(variable-length)字节序列(a squence of 8-bit bytes)储存一个字符,

    序列的长度范围在 1 到 5 个字节内(包括5). =ASCII= 字符只需要 1 个字节, =NON-ASCII= 字符则大于 1 个字节.

    因为并不是所有文件都是用的 *Unicode Standard* 储存

    每次 =Emacs= 打开文件都需要先把文件内容写入到{{{buffer}}}中,这时候的文件内容叫做 =encoded text=,它不是真的 =text=,它是一个由多个独立字节排列成的序列,

    然后根据文件储存时的编码标准把 =encoded text= 转换为 =Emacs= 编码标准能够解析的结构(另外一个种形式的字节序列),文件内容就显示为平时看到的文本,

    这编码之间的转换叫做编码转码(character code conversion);

    保存文件则是上面的逆过程: 根据 =Emacs= 编码标准把{{{buffer}}}的内容转换为文件的编码标准所能够解析的结构,也就是转回 =encoded text=,最后写入到文件中.

    =Emacs= 也可以不对文件进行转码显示,允许用户在这种状态下对文件进行编辑,通常情况是对非本文文件进行编辑.

    在 =Emacs= 里,这种直接储存 =encoded text= 的{{{buffer}}}和字符串叫做 =unibyte= {{{buffer}}}和字符串,
    ...
    请求得到的内容是字节流,英文字符是正常显示但中文字符是八进制的,因为一个英文字母字只要占一个字节,而一个中文字符需要多个字节,所以这个{{{buffer}}}是 =unibyte= {{{buffer}}},

    直接调用 =libxml-parse-html-region= 会解析不出正确结果,因此要调用 =set-buffer-multibyte= 把 {{{buffer}}} 变成 =multibyte= {{{buffer}}} 正常显示出文本.
