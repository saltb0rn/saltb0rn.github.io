...
*** Calc
    ...
    就是因为 =Calc= 是太复杂了,时刻查阅文档是必须的,通常都是忘记按键以及命令,那么这文档是很常用的: *计算器命令索引* =M-: (info "(Calc)Command Index")=.

    大部分情况都是知道要用什么命令做什么计算,但是不知道命令对应的按键序列索引,

    这可以在 *计算器命令索引* 中通过 =\C-s= 查找出命令,再反查出命令对应的按键,

    这完全是得益于 =Calc= 的开发者遵守 *语义一致* 以及 *定义的名字符合其含义* 的原则.
