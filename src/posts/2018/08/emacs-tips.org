...
*** 线程 (Threads)
    ...
    通信必然需要考虑一个 *先后* 问题,就拿一个假设的例子来看: 现有两个线程,其中一个线程修改某个内存地址,另外一个线程访问这个内存地址来获得修改后的值.
...
*** Emacs 编码系统
    ...
    *UTF-16* 和 *UTF-32* 都以多字节作为基本编码单位,这就需要讨论一下多字节编码的排序问题了,
    ...
    这其实不是文本文件特有的问题,所谓的大端就是按照从低位往高位方向 *增长* 的顺序排列 *一个数据* 的 *基本单位*,也就是从高位到低位顺序排列,一般往左边就是高位方向,

    就是从左到右排序,比如 10 进制数字 1230, 1 是千位, 2 是百位, 3 是十位, 0 是个位, 1230 就是基本数据,它的基本单位就是数位,这种排列顺序符合大端的定义;

    小端则是相反,从高位往低位方向 *增长* 排列 *一个数据* 的基本单位, 1230 的小端排列就是 0321.

    但计算机可没有这么简单,这是因为基本单位的选取是很复杂的,基本单位可以是位(bit),也可以是字节(byte),

    特别是 *UTF-16* 和 *UTF-32* 的情况相对 *UTF-8* 复杂,为了讲明白这大小端在计算机中是什么的一个概念,先抛开文本编码从一个简单的例子说起:
    ...
    所以,判断数据是使用大端还是小端编码对于字节流分析来说是很重要的,如果这里总结得不够清楚可以看 [[https://www.digital-detective.net/understanding-big-and-little-endian-byte-order/][understanding big and little endian byte order]].
