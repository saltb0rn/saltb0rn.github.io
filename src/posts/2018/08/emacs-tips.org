...
*** 线程 (Threads)
    ...
    就拿一个假设的例子来看: 现有两个线程,其中一个线程修改某个内存地址,另外一个线程访问这个内存地址来获得修改后的值.

    但是因为是抢占式调度,如果一个线程在修改到一半时被强迫让出 =CPU=,这个时候被修改的内存是没有被释放的,另外的线程获取到的就是修改前的值.

    为此必须要确保线程们的执行顺序能够正确完成任务,避免两个或者更多的线程同时访问/修改同一个内存地址,这有一个叫法: 保证 *线程同步*.
...
*** Emacs 编码系统
    ...
    *UTF-16* 和 *UTF-32* 都以多字节作为基本编码单位,这就需要讨论一下多字节编码的排序问题了,
    ...
    这其实不是文本文件特有的问题,先抛开计算机不谈,大端和小端就是排列的顺序而已.

    大端就是按照从低位往高位方向 *增长* 的顺序排列 *一个数据* 的 *基本单位*,这里高位和低位单纯是指位置,从高位到低位方向一般规定为从左往右的方向,

    就拿 10 进制数字 1230 来说,他的数位排列位 *千百十个*,每当某个数位上的数值满 10 就要往高一级的数位进 1, 比如把个位的数值改为 12,

    那么这个数字就变成 1242, 这种排序就是上面说的大端; 按照上面大端的定义,我们可以把 1230 这个数字叫做一个数据,它的基本单位就是数位;

    小端则是相反,从高位往低位方向 *增长* 排列 *一个数据* 的基本单位,这里的 1230 的小端排列就是 0321,这里最高位不再是千位而是个位了,

    整个数字的数位顺序变成 *个十百千*, 每当个位满 10 就要往低一级的数位进 1,这里同样把个位改成 12,那么整数字就变为 2421.

    回到计算机中,常用基本单位不是字节(byte)就是位(bit), 通常文件储存和传输都是以字节作为数据的 *基本单位* 的,

    具体 *一个数据* 是什么,这要看情况,一个文件本身也符合一个数据这个说法,而这个数据本身也是由很多个更小的 *一个数据* 组成,
    ...
    虽然我个人觉得上面已经说得很清楚了,但我还是要说明一下,在以字节作为 *基本单位* 的情况下,

    不管是大端还是小端,字节自身里面的位的顺序还是大端的.

    在上面的三种编码标准中, 其中 *UTF-16* 和 *UTF-32* 都涉及大小端的概念,并且文本文件编码过程相对复杂,

    而以它们作为开场例子来讲明白这大小端在计算机中是什么的一个概念是不太合适的,因此先抛开文本编码从一个简单的例子说起:

    现在需要把 1230 作为一个数据储存到计算机里面,我们假设计算机编码数字的原理是把数字的二进制形式和数字本身等同起来,并且规定一个字节为一个基本单位,
    ...
    如果觉得这里总结得不够清楚的话,可以看 [[https://www.digital-detective.net/understanding-big-and-little-endian-byte-order/][understanding big and little endian byte order]].
