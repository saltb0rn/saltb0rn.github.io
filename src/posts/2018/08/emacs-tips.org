...
*** Emacs 编码系统
    ...
    *UTF-16* 和 *UTF-32* 都以多字节作为基本编码单位,这就需要讨论一下多字节编码的排序问题了,
    ...
    小端则是相反,按照 $低位 \longrightarrow 高位$ 的顺序把 *一个数据* 的 *基本单位* 从不太重要到重要的顺序进行排列,

    所以 1230 的小端就是 0321.

    这两者的区别就像不同地方的人用不同顺序写字一样,比如中国古人是从右边往左的方向写字,而现在的中国人则是从左往右的方向写字,

    从左边开始写,左边就重要;从右边开始写,那么右边就总要.

    要注意,这里的 *高位* 和 *低位* 只是单纯指位置,也不一样是往左方向就是低位方向,完全是人为定义的,

    计算机的高位和低位可不是指位置,计算机的高位指内存高位,低位就是内存低位.
