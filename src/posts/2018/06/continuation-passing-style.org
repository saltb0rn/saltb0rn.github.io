...
=CPS= 虽然很好,但如果把它直接用在像 =Python= 和 =Emacs Lisp= 这种有”先天缺陷“ (不支持尾递归优化 Tail Call Optimization, abbr. TCO) 的语言上面就不行了,不过既然改成 =CPS= 了,

那就可以把 =CPS= 转化成迭代/循环,并且 =Python= 和 =Emacs Lisp= 这些语言都有循环语句,可以很好避免爆栈的缺陷.
