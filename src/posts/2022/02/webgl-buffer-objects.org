...
#+begin_src javascript
  ...
  const mask = 0x01               // [0, 255]
  stencilMask(mask) // 当 mask = 0 时, 模板缓冲被禁止更新; 其它情况就是按照

  const func = ALWAYS
  const ref = 0x01
  stencilFunc(func, ref, mask) // 设置模板测试的方式
  /*
    @func 测试用的比较运算符, 对参考值 (ref & mask) 和 (stencil & mask) 进行比较, 有以下选择


    NEVER: 永不通过
    ALWAYS: 总是通过
    EQUAL: 当 (ref & mask) == (stencil & mask) 成立, 测试通过
    NOTEQUAL: 当 (ref & mask) !== (stencil & mask) 成立, 测试通过
    LESS: 当 (ref & mask) < (stencil & mask) 成立, 测试通过
    LEQUAL: 当 (ref & mask) <= (stencil & mask) 成立, 测试通过
    GREATER: 当 (ref & mask) > (stencil & mask) 成立, 测试通过
    GEQUAL: 当 (ref & mask) >= (stencil & mask) 成立, 测试通过
  ...
  /*
    ...
    KEEP: 保持当前 stencil
    ZERO: 清零 stencil = 0
    REPLACE: 使用 stencilFunc 中的 ref 并配合 mask 进行替换, stencil = (ref & mask)
    INCR: 对当前 stencil++, 当 stencil 大于 255, 那么 stencil 就被限定为 255
    INCR_WRAP: 对当前 stencil++, 当 stencil 大于 255, 那么 stencil = 0
    DECR: 对当前 stencil--, 当 stencil 小于 0, 那么 stencil = 0
    DECR_WRAP: 对当前 stencil--, 当 stencil 小于 0, 那么 stencil = 255
    INVERT: stencil = ~stencil, 比如
  ,*/
  ...
  stencilFunc(EQUAL, 0x01, 0xFF) // (0x01 & 0xFF) == (stencil & 0xFF)
  ...
  /*
    ...
    其中片元 a 的测试为 (0x01 & 0xFF) !== (0x00 & 0xFF), 测试不通过, 所以 a 不能显示在屏幕上;

    而 b/c/d 的测试都为 (0x01 & 0xFF) == (0x01 & 0xFF), 测试通过, 所以 b/c/d 会出现在屏幕上;
