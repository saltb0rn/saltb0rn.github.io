...
也因为这样, 图形学学习计划搁置了一年, 当时就停留在贴图映射以及 =WebGL= 的掌握阶段.
...
而在整个渲染流程里面, 开发人员基本上只会关心其中的 =Vertex Shader= 和 =Fragment Shader= 两个阶段.
...
因为 =WebGL= 的渲染流程是老版本的 =OpenGL= 的渲染流程, 相差了几个过程,

为了符合 =WebGL= 的流程, 我只关注了新版渲染流程中的 =Vertex Shader= 和 =Fragment Shader= 及其前后的几个阶段.

相比开始找到的教程来说只是针对 =Fragment Shader= 进行一些细节上补充, 最后, 在阅读这篇笔记时记得配合官方流程说明进行理解, 并以它为准.

这里我们会先讲解工作流程的大概, 后面会提供一个可运行的例子来讲解一些细节上的东西.
...
因为 =OpenGL= 只知道顶点的信息, 要根据这些点画出一个三角形,

就得使用插值(interpolation)的上采样(upsampling)方法来补充顶点之间的点, 从而构造出由方块组成的近似三角形.
...
因此在看的时候要紧记 *OpenGL 的光栅化阶段不会计算出片元的颜色*, 这是 =Fragment Shader= 的工作, =OpenGL= 的光栅化的 *主要* 工作是确定片元的位置, 证据就在这[fn:1:[[https://www.khronos.org/opengl/wiki/Fragment_Shader#Optional][Fragment Shader is Optional]]].

此外, 光栅化还会对一些其它变量进行插值并输出到下个阶段, 比如在 =OpenGL (ES) 2= 之前的 =varying= 变量[fn:2:[[../../../files/GLSL_Programming_Rasterization.pdf][GLSL Programming Rasterization]]], 我会在 =Fragment Shader= 中介绍它.
#+END_QUOTE
...
#+BEGIN_QUOTE
在下面的绘制三角形的例子里面, =vertex shader= 根据三角形的顶点数据的坐标为它们定义颜色(, 这样每个顶点的颜色就不一样了), 并把颜色保存到 =varying= 变量 =vColor= 里面.

在光栅化过程中, =OpenGL= 除了会根据顶点数据计算片元序列 $f_{1}, f_{2}, f_{3}, \ldots, f_{n-1}, f_{n}$ 外,

同时还会根据与顶点数据一起定义的 =varying= 变量进行插值得到序列 $v_{1}, v_{2}, v_{3}, \ldots, v_{n-1}, v_{n}$,

并且片元 $f_{i} (i \le n)$ 和 $v_{i} (i \le n)$ 是相关的, 这样就相当于每个片元都有属于自己的 =varying= 变量.

在 =fragment shader= 把片元自己的 =vColor= 设置为自己的颜色, 最后得到一个颜色渐变的三角形.
