...
*** WebGL是如何工作的
    ...
    经过几个阶段后到达光栅化阶段, 该阶段会产生一种叫做片元(=fragments=)的数据, 片元就是一种类像素数据(=pixel-like data=),

    =Fragment Shader= 会处理这些片元数据,生产像素,而像素就是一个色值(RGB/RGBA).
    ...
    每当光栅化得到一个片元, =Fragment Shader= 就开始对这个像素点进行处理.
