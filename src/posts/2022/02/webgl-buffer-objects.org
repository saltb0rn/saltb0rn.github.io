...
#+begin_quote
想了解如何在透视投影中对片元的任意属性进行插值, 可以阅读 [[../../2020/06/graphics-opengl-transformation.html#arbitrary-attr-interp][Perspective-Correct Interpolation]].
#+end_quote
...
这里就讨论剪裁测试(scissor test), 模板测试(stencil test / stencil buffer test) 以及深度测试(depth test / z buffer test) 这三个测试(这里是它们的先后顺序).

其中剪裁测试以及深度测试都比较好理解, 裁剪测试是丢弃剪裁框范围外的片元,

深度测试是丢弃那些被其它片元挡住的片元, 留下没被遮挡住的片元.

其中模板测试和深度测试是开发者需要重点理解的,

#+attr_html: :width 600px
#+caption: 模板测试以及深度测试流程
[[../../../files/stencil-buffer-z-buffer-test.jpg]]

#+begin_src javascript
  const fragments = [{ x, y, z }, ... ]
  // (x, y) 是像素坐标, z 是深度值
  for (let { x, y, z } of fragments) {
    if (stencilTest()) {
      if (depthTest()) {
        RenderPixel(x, y);        // 把片元输出到屏幕中
        DepthBufferOp(z);         // 更新深度缓冲
        StencilOpZPass();         // 通过模板测试以及深度测试时, 更新模板缓冲
      } else {
        StencilOpZFail();         // 通过模板测试但深度测试不通过时, 更新模板缓冲
      }
    } else {
      StencilOpFail();            // 模板测试不通过时, 更新模板缓冲
    }
  }
#+end_src

接下来会分开了解模板测试以及深度测试的具体逻辑, 先从较为简单的深度测试开始.
...
#+begin_src javascript
  ...
  // 深度测试以及更新深度缓冲
  function updateDepthBufferIfCloser(x, y, invZ, depthUpdate) {
    /*
      x: int
      y: int
      inv_z: float
  ...
  for (let y = p0.y; y <= p2.y; y++) {
    ...
    for (let x = xl; x <= xr; x++) {
      ...
      // 进行深度测试, 并且更新深度缓冲
      if (depthTestEnable &&
          updateDepthBufferIfCloser(x, y, iz, depthMask)) {
        continue;
      }
...
模板测试可以理解为位屏蔽(bit masking)的类似, 使用一个模板缓冲(stencil buffer)作为遮罩层, 把模板值不符合条件的片元都丢弃,

为了更直观地理解, 我直接从 [[https://learnopengl.com/Advanced-OpenGL/Stencil-testing][LearnOpenGL]] 上把图给扒下来, 来看看模板测试以及模板缓冲是怎么样的:

#+CAPTION: 模板测试: 未启用模板测试(左), 要使用的模板缓冲(中), 启用模板测试后(右)
...
模板测试的重点在于它的 =API= 使用流程, 各种渲染引擎仍然采用这套 =API= 设计.

#+begin_src javascript
  // 渲染管线中有一个默认的模板缓冲, 这些 API 都是这对这个缓冲进行更新

  enable(STENCIL_TEST)            // 开启模板测试

  const writeMask = 0xFF          // [0, 255]
  stencilMask(writeMask) // 当 writeMask = 0 时, 模板缓冲被禁止更新; 其它情况就是按照 value & writeMask 写入缓冲
  /* 需要 stencilMask 的参数名并非为 writeMask 而是 mask,

     但接下来的 stencilFunc 也有一个叫做 mask 的参数.

     虽同名, 但作用完全是不一样的, stencilMask 的 mask 是控制写入, stencilFunc 的 mask 是控制读取,

     因此分别为它们重命名为 writeMask 和 writeMask.

     虽然 *通常* 两者的值都是一样的, 但作为开发者要明白它们的区别.
  ,*/

  const func = ALWAYS
  const ref = 0x01
  const readMask = writeMask
  stencilFunc(func, ref, readMask) // 设置模板测试的方式
  /*
    @func 测试用的比较运算符, 对参考值 (ref & readMask) 和 (stencil & readMask) 进行比较, 有以下选择


    NEVER: 永不通过
    ALWAYS: 总是通过
    EQUAL: 当 (ref & readMask) == (stencil & readMask) 成立, 测试通过
    NOTEQUAL: 当 (ref & readMask) !== (stencil & readMask) 成立, 测试通过
    LESS: 当 (ref & readMask) < (stencil & readMask) 成立, 测试通过
    LEQUAL: 当 (ref & readMask) <= (stencil & readMask) 成立, 测试通过
    GREATER: 当 (ref & readMask) > (stencil & readMask) 成立, 测试通过
    GEQUAL: 当 (ref & readMask) >= (stencil & readMask) 成立, 测试通过

    @ref 参考值

    @mask 对参考值 ref 和模板值 stencil 的位屏蔽码
  ,*/
  ...
  const zfail = KEEP
  const zpass = REPLACE
  stencilOp(fail, zfail, zpass)   // 在不同测试结果下, 对模板缓冲的更新动作
  /*
    @fail StencilOpFail
    @zfail StencilOpZFail
    @zpass StencilOpZPass

    KEEP: 保持当前 stencil
    ZERO: 清零 stencil = 0
    REPLACE: 使用 stencilFunc 中的 ref 并配合 mask 进行替换, stencil = (ref & writeMask)
    INCR: 运算 ++stencil, 当运算后的 stencil & writeMask 大于 255, 那么 stencil 就被限定为 255
    INCR_WRAP: 运算 ++stencil, 当运算后的 stencil & writeMask 大于 255, 那么 stencil = 0
    DECR: 运算 --stencil, 当运算后的 stencil & writeMask 小于 0, 那么 stencil = 0
    DECR_WRAP: 运算 --stencil, 当运算后的 stencil & writeMask 小于 0, 那么 stencil = 255
    INVERT: stencil = ~stencil,
  ,*/

  depthMask(false)                // 关闭深度缓冲更新, 避免绘制蒙版用途的物体对深度缓冲造成影响
  clear(STENCIL_BUFFER_BIT)       // 模板缓冲默认值是不确定的, 因此这里手动清除缓冲可以使得默认值固定为0

  /*

    假设输出图像是 4 x 4 的分辨率, 清除过后的模板缓冲的数据如下
  ...
  /*
    ...
    那么蒙版物体应该占图像中间的四个像素(- 代表空白区域, + 代表物体的片元):
    ...
    也就是说物体片元在平面中的位置为 (x, y), 那么模板缓冲在 (x, y) 上的模板值就是该片元的模板值

    然后调用绘制命令, 绘制充当蒙版用途的物体 maskMesh
  ,*/
  drawElements(maskMesh)

  /*
    在调用绘制命令后, 会按照前面的设置更新模板缓冲.

    现在可以使用这个缓冲作为后续绘制的物体的蒙版.

    但首先要保证后续绘制命令不会修改当前的模板缓冲,

    所以要关闭缓冲更新.
  ,*/
  stencilMask(0x00)

  // 在绘制前, 要先决定好接下来模板测试的方式
  stencilFunc(EQUAL, 0x01, readMask) // (0x01 & readMask) == (stencil & readMask)
  // 由于关闭了模板测试更新, 所以不需要设置 stencilOp 来调整模板更新的动作
  // 开启深度缓冲更新
  depthMask(true)

  /*
    绘制物体, 未启用模板测试时, 新物体 targetMesh 的片元 a,b,c,d 在的排列如下:
    ...
    根据测试设置, 在启用测试户, 此时物体片元参考值为 0x01,

    对比模板缓冲, a/b/c/d 这4个位置上的模板值分别为 0x00, 0x01, 0x01, 0x01

    其中片元 a 的测试为 (0x01 & readMask) !== (0x00 & readMask), 测试不通过, 所以 a 不能显示在屏幕上;

    而 b/c/d 的测试都为 (0x01 & readMask) == (0x01 & readMask), 测试通过, 所以 b/c/d 会出现在屏幕上;
...
#+begin_src javascript
  ...
  // 获取定向光源的深度贴图
  {
    ...
    // dirLight.shadow.matrix 从世界坐标变换到纹理坐标的矩阵, 配合阴影贴图使用
  }
  ...
  // 获取点光源的深度贴图
  {
    ...
    // pointLight.shadow.matrix 从世界坐标变换到纹理坐标的矩阵
  }
#+end_src

在开发中, =three.js= 的阴影贴图通常是在内部使用的, 但不排除有些情况需要用到,

而 =three.js= 的文档并没有详细地给出阴影贴图的信息.

首先, =three.js= 的阴影贴图储的是 $[0, 1]$ 且经过编码的线性深度, 需要解码后才能使用.

其次, 点光源 (=PointLight=) 阴影贴图并非立方体贴图 (=Cubemap=), 并非 =2D= 贴图,

在采样前时需要根据光照方向计算出阴影贴图中的 =uv= 坐标.

具体参考 [[../../2020/08/graphics-opengl-light-and-material.html#no-cube-map-shadow-mapping][OpenGL - 光和材质: 位置光源的阴影 - 不使用 CubeMap]] (必读).

以下是采样阴影贴图时要用到的 =three.js= 内置函数:
...
接下来重点讲解一下如何在 =Shader= 中正确地使用阴影贴图.
...
#+begin_src glsl
  ...
  uniform sampler2D tShadowMap;    // Three.PointLight.shadow.map.texture
  uniform vec2 uShadowMapSize;     // Three.PointLight.shadow.mapSize
  uniform vec3 uLightPos;          // Three.PointLight.position
  uniform float uShadowCameraNear; // Three.PointLight.shadow.camera.near
  uniform float uShadowCameraFar;  // Three.PointLight.shadow.camera.far

  void main() {
    vec3 lightToFrag = vWorldPos - uLightPos;
    vec3 lightDir = normalize(lightToFrag);
    float texelSizeY = 1.0 / (uShadowMapSize.y * 2.);

    float currentDepth = (length(lightToFrag) - uShadowCameraNear) / (uShadowCameraFar - uShadowCameraNear);
    float cloestDepth = unpackRGBAToDepth(texture2D(tShadowMap, cubeToUV(lightDir, texelSizeY)));
    float shadow = currentDepth - 0.001 >= cloestDepth 1.0: 0.0;
    gl_FragColor = mix(vec4(1.), vec4(0.2), shadow);
  }
#+end_src
...
#+begin_src glsl
  ...
  uniform sampler2D tShadowMap;               // Three.DirectionalLight.shadow.map.texture
  uniform mat4 uShadowMatrix;                 // Three.DirectionalLight.shadow.matrix
  // Three.DirectionalLight.shadow.matrix 是直接从世界坐标变换到纹理坐标
  // https://github.com/mrdoob/three.js/blob/master/src/lights/LightShadow.js

  void main() {
    vec4 posInLightSpaceUVW = uShadowMatrix * vec4(vWorldPos, 1.);
    float currentDepth = posInLightSpaceUVW.z;
    float cloestDepth = unpackRGBAToDepth(texture2D(tShadowMap, posInLightSpaceUVW.xy));
    float shadow = currentDepth - 0.001 >= cloestDepth 1.0: 0.0;
