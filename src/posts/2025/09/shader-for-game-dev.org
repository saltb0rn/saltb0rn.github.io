...
2. [[../../2020/06/graphics-opengl-transformation.html][图形学 - OpenGL坐标系变换]]
...
5. [[../../2024/03/code-explains-for-fragment-shader-in-shadertoy.html][ShaderToy常见代码解析]]
   ...
   有很多人说 =ShaderToy= 的代码对游戏开发没有帮助, 其实是不对的, 前面举例的这些知识点很常见,

   我们需要学会如何把问题分解成多个更小的问题, 再利用这些知识去解决小问题, 从而解决原本的问题.
...
根据 [[../../2020/06/graphics-opengl-transformation.html#depth-buffer][图形学 - OpenGL坐标系变换: 透视投影 - Depth Buffer]] 可得知, 深度贴图的像素用于储存深度值,
...
换而言之, 透过水面观察到的景象(有水面景象)与移除水面直接观察到的景象(无水面景象)相比, 两者只是发生了轻微偏移,

这意味着折射射线从无水面景象进行采样作为有水面景象. 因此, 场景贴图的生成思路大概如下:
...
部分光线聚焦(=converging=)在相同位置上, 部分光线则是被发散(=diverging=)在不同位置上,
...
$\frac{A(t_1)}{A(t_2)} \gt 1$ 说明发生了聚焦, $\frac{A(t_1)}{A(t_2)} \lt 1$ 说明发生了发散.
...
这一部分要求读者熟练掌握 [[../../2020/06/graphics-opengl-transformation.html][图形学 - OpenGL坐标系变换]] 的内容.
...
整个合成过程不直接操作 =3D= 场景, 而是基于渲染得到的 =2D= 图像进行后处理.

#+attr_html: :width 700px
#+caption: 后处理流程, 这里称"把内容输出到屏幕上的相机"为 *后处理相机*, 称"平面 $P$"为 *后处理平面*
[[../../../files/postprocessing.png]]

在 =2D= 空间上完成后处理并不意味着无法读取到 =3D= 场景的几何信息,

在开始后处理之前, 使用对应的 =shader= 程序应用在场景物体上, 进行渲染并把渲染结果保存在不同贴图上,
...
这里先定义一个概念: 一个顶点坐标在经过相机投影所得的 =NDC= 坐标被称为相机 =UV=.

相机 =UV= 是用来从相机生成的 =G-Buffer= 获取数据的,

*一般* 是哪个相机生成的 =G-Buffer= 就得用哪个相机的 =UV= 去获取数据.

而模型的几何数据通常也有一个名为 =uv= 的 =attribute=, 这里把这个 =attribute= 称为材质 =UV=.

通常在建模时模型都会配备一张贴图用于着色, 材质 =UV= 就是用来从这样贴图上获取数据用于着色,

这里把这个贴图称为 *材质贴图*, 材质 =UV= 与材质贴图之间的映射关系 *通常* 是在建模时就定好的.

换而言之, 一个顶点坐标可以同时有两类 =UV= 坐标, 这也导致 *初学者很容易在编写 =shader= 时用错 =UV= 进行采样.*

#+begin_quote
为了减少这种问题的发生, 应该养成采样前自检的习惯, 第一个问题:

*贴图是否为材质贴图?*

如果是, 那么就是用材质 =UV= 采样; 否则, 进入下一个问题:

*贴图由哪个相机生成的?*

找出对应相机, 并计算出对应的相机 =UV= 坐标用于采样.
#+end_quote

接下来演示一个例子.

比如想从 =tGBufferA= 上获取几何信息, 可以让 =Camera A= 生成保存了视点坐标的位置贴图 =tPositionView=,

=Camera A= 的投影矩阵为 =projectionMatrixA=, =Camera O= 为后处理相机,

我们可以用 =Camera O= 的相机 =UV= 从 =tPositionView= 中获取视点坐标 =pView=,

再根据 =pView= 和 =projectionMatrixA= 计算出 =pView= 在 =Camera A= 的相机 =UV=,

利用该相机 =UV= 从 =tGBufferA= 中获取数据:

#+BEGIN_SRC glsl
  // 合成最终图像的 fragment shader
  uniform vec2 uResolution;        // 最终画面的分辨率
  uniform mat4 projectionMatrixA;  // Camera A 的投影矩阵
  uniform sampler2D tGBufferA;     // G-Buffer A
  uniform sampler2D tPositionView; // 保存了视点坐标的位置贴图, 由 Camera A 生成

  vec2 ssUV = gl_FragCoord.xy / uResolution.xy; // 最终画面的 UV 坐标, 是材质 UV
  vec4 pView = texture2D(tPositionView, ssUV);
  /* 根据后处理流程, G-Buffer 本质上就是后处理平面的材质贴图,
     因此, 即便 tPositionView 不是由 Camera O 生成的,
     这里依然可以用后处理平面的材质 UV 进行采样 */
  vec4 pClip = projectionMatrixA * pView;
  vec3 pNDC = pClip.xyz / pClip.w;
  vec3 pUVW = pNDC * 0.5 + 0.5;   // 片元在 Camera A 的相机 UV
  vec4 pGinfo = texture2D(tGBufferA, pUVW.xy); // tGBufferA 是 Camera A 生成的, 所以得用它的相机 UV
#+END_SRC

可以看出不同相机生成的位置贴图 =tPositionView= 的重要性, 它能为同一个相机生成的不同 =G-Buffer= 提供相机 =UV=;

在屏幕空间技术中, 后处理相机的 =UV= 坐标可以说是一切的起点.
...
计算方法可以参考 [[../../2020/06/graphics-opengl-transformation.html#normal-texture][图形学 - OpenGL坐标系变换: 法线贴图]] 里面的 =TBN= 矩阵计算, 有提及到如何根据顶点计算出法线/切线以及副切线.
#+end_quote
...
#+caption: 反射光线 $R_1$ 与场景的交点 $P$ 在相机前, 但正确的反射内容在相机背后
[[../../../files/ssr-ir-diag-5.png]]

可以现这两种情况都是入射射线 $I$ 与反射射线 $R$ 之间的夹角 $\theta \in [0^{\circ}, 180^{\circ}]$ 在 $[0^{\circ}, 90^{\circ}]$ 内才出现的,

因此, 当 $\theta \in [0^{\circ}, 90^{\circ}]$ 时, $\cos \theta \ge 0$, 说明 $R$ 是否往近裁剪平面方向移动.
  ...
  for (float i = 0; i < stepCount; i++) {
    ...
    if (currentViewZ <= samplingViewZ) {
      ...
      if (hit) {
        ...
        // 保证反射光线是向着嫌疑交点所处表面射去, 而不是往近裁剪平面射去
        if (dot(endPosView - startPosView, samplingNormalView) >= 0.0) continue;
        /*
...
假设 $I$ 和 $R$ 是单位向量, $I$ 是视线在近裁剪平面法线的投影方向, $R$ 是反射光线的方向,

#+begin_quote
为了方便作图, 这里假设视线垂直于近裁剪平面, 使得 $I$ 与视线共线.

实际上不管视线是否垂直于近裁剪平面, $I$ 都是不变的,

也就是说视线是否垂直于近裁剪平面都不影响接下来的讨论.
#+end_quote

可得到 $\begin{cases} \overrightarrow{OB} = O + t \times R \\ |\overrightarrow{OB}| = t \times |R| \end{cases}$, 只要求出 $t$ 就能求出 $B$.
...
根据几何关系, $|I| \times \cos(\angle AOB)$ 是 $R$ 在 $I$ 上的投影长度, 正好等于 $R$ 在视点空间上的 $z$ 分量 $(R_{view})_z$;
...
最终 $t = \frac{|\overrightarrow{OA}|}{|I| \times \cos(\angle AOB)} = \frac{-n - (O_{view})_{z}}{(R_{view})_{z}}$.
    ...
    for (float i = 0.; i < maxComp; i++) {
      ...
      if (currentPosViewZ <= samplingPosView.z) {
          ...
          if (hit) {
            ...
            // 保证反射光线 reflectView 是向着场景的交点 samplingPosView 所处的表面射去, 而不是往近裁剪平面射去
...
生成场景贴图, 对场景贴图中每个区域的像素进行亮度筛选, 当通过筛选, 那么该像素就纳入统计, 得出该区域内的平均亮度.
...
生成视点空间的顶点贴图, 根据后处理相机的 =UV= 从顶点贴图中获取视点坐标 $p_1$,

再根据视点坐标还原出上一帧时该顶点的视点坐标 $p_0$.

$p_1 - p_0$ 可以得出运动方向, 可以作为模糊方向.
...
生成场景贴图作为 =FocusTexture=, 使用 =Dilation= 算法生成 =FocusOutTexture=.

以指定后处理相机的 =UV= 为圆心计算出一个圆, 圆内区域显示 =FocusTexture= 的内容,

圆外区域显示 =FocusOutTexture= 的内容.

*** 体积光照 (Volumetric Lighting)

体积光照是指模拟光线在进入雾/尘埃等介质所产生的散射现象.

**** 整体思路

当物体出现在视野中时, 计算从物体到相机(视线)的过程中经过了多少个光源,

当视线步进第 $n$ 次经过光源时, 就以某种方法统计出 *视线受光源影响的程度: $I_n$* 并进行累计 $S = \sum\limits_{i=1}^{n}$,

当整个视线步进在第 $N$ 步程结束, 求出 $I = \frac{S}{N}$, 并以 $I$.


#+attr_html: :width 600px
#+caption: 点光源的体积光
[[../../../files/god-ray-spotlight.png]]

#+attr_html: :width 600px
#+caption: 平行光源的体积光
[[../../../files/god-ray-parallel.png]]
...
*** 为光栅化场景生成有向距离场 (Generate SDF in Rasterization)

https://github.com/gkjohnson/three-mesh-bvh

https://github.com/gkjohnson/three-mesh-bvh/tree/master?tab=readme-ov-file#bvh-queries-in-a-shader

https://github.com/gkjohnson/three-mesh-bvh/blob/master/example/gpuPathTracingSimple.js

https://github.com/gkjohnson/three-mesh-bvh/blob/91be8eb38b434835108e435b07312113b3aeded3/src/webgl/BVHShaderGLSL.js#L3
...
先生成 =BVH=, 再根据 =BVH= 生成 =SDF=.

比起 =three-mesh-bvh= 更简单的 =BVH= 实现

https://github.com/benraziel/bvh-tree?tab=readme-ov-file

=three-mesh-bvh= 生成 =SDF= 的算法:

https://github.com/gkjohnson/three-mesh-bvh/blob/master/example/sdfGeneration.js

=updateSDF= 的 =CPU= 生成.

**** 计算判定点与三角形所处平面的有向距离

假设判定点 $Q$ 与三角形 $\Delta ABC$ 不处于同一个平面,

首先需要清楚平面的朝向, 它等于三角形的法向量:

$\vec{N} = \overrightarrow{AB} \times \overrightarrow{AC}$, 其中 $\begin{cases} \overrightarrow{AB} = B - A \\ \overrightarrow{AC} = C - A \end{cases}$

其单位向量为 $\hat{N} = \frac{\vec{N}}{\left| N \right|}$,

点 $Q$ 到平面的有限距离为 $d = \overrightarrow{AP} \cdot \hat{N}$,

点 $Q$ 在平面上的投影点为 $P = Q - d \times \hat{N}$.

**** 判断投影点是否在三角形内部 - 重心坐标法(Barycentric Coordinate Method)

判断点是否在三角形内有很多种方法, 比较常用的有同向法和接下来要学习重心坐标法.

已知 $P$ 是 $Q$ 在三角形 $\Delta ABC$ 所处平面上的一个投影点,

同向法的原理要比重心坐标法更加直观一点, 但有适用条件: 需要提前确定三角形顶点的渲染顺序.

相比之下, 重心坐标法是所有情况通用的. 其实如果对线性代数有了解的话, 重心坐标法也是挺直观的:

把 $\overrightarrow{AB} = B - A$ 和 $\overrightarrow{AC} = C - A$ 分别看作坐标轴, 以 $A$ 为原点共同构成一个平面坐标系,

$(u, v)$ 就是 $P$ 该坐标系上的一个线性组合

$\begin{equation*}\begin{aligned}
& P = A + u \overrightarrow{AC} + v \overrightarrow{AB} \\
= & A + u (C - A) + v (B - A) \\
= & (1 - u - v) A + vB + uC \\
\rightarrow & P - A = \overrightarrow{AP} = u \overrightarrow{AC} + v \overrightarrow{AB}
\end{aligned}\end{equation*}$

[[../../../files/barycoord.png]]

令 $\begin{cases} \beta = v \\ \gamma = u \\ \alpha = 1 - u - v = 1 - \beta - \gamma \end{cases}$, 得到的 $P = \alpha A + \beta B + \gamma C$ 就是常见的重心坐标定义.

当 $\begin{cases} v + u \le 0 \\ v \gt 0 \\ u \gt 0 \end{cases}$ 同时满足, 那就 $P$ 处于三角形内; 否则 $P$ 在三角形之外, 这就是重心坐标法的原理.

如果不理解, 可以想象一下 $v \lt 0$, $P$ 就处于 $\overrightarrow{AC}$ 反方向, 这必然在三角形之外;

如果 $\begin{cases} v = 1.5 \\ u = 0\end{cases} \rightarrow v + u \gt 1$, $P$ 就超出三角形的范围.

#+begin_quote
还可以从从三角形面积的角度出发, 我们知道 $\vec{a} \times \vec{b}$ 模长等于 $\vec{a}$ 和 $\vec{b}$ 所张成的平行四边形面积,

而三角形的面积正是平行四边形的一半. 我们可以让点 $P$ 分别与点 $A$ / $B$ / $C$ 之间连接线段,

构造出 $\Delta APB$ / $\Delta APC$ / $\Delta BPC$ 三个子三角形: $S_{\Delta APB} + S_{\Delta APC} + S_{\Delta BPC} = S_{\Delta ABC}$.

接下来分别计算出它们的面积关系:

$S_{\Delta ABC} = \frac{1}{2}\left| \overrightarrow{AC} \times \overrightarrow{AB} \right|$

$\begin{equation*}\begin{aligned}
& S_{\Delta APB} \\
= & \frac{1}{2}\left| \overrightarrow{AP} \times \overrightarrow{AB} \right| \\
= & \frac{1}{2}\left|(u\overrightarrow{AC} + v\overrightarrow{AB}) \times \overrightarrow{AB}\right| \\
= & \frac{1}{2}\left| u \overrightarrow{AC} \times \overrightarrow{AB} + v \overrightarrow{AB} \times \overrightarrow{AB} \right| && \text{应用向量运算性质} (\vec{a} + \vec{b}) \times \vec{c} = \vec{a} \times \vec{c} + \vec{b} \times \vec{c} \\
= & \frac{1}{2}\left| u \overrightarrow{AC} \times \overrightarrow{AB} \right| && \text{应用向量运算性质} \vec{a} \times \vec{a} = \vec{0} \\
= & \frac{1}{2} \left| u \right| \left| \overrightarrow{AC} \times \overrightarrow{AB} \right|
\end{aligned}\end{equation*}$

$S_{\Delta APC} = \frac{1}{2}\left| \overrightarrow{AC} \times \overrightarrow{AP} \right| = \left| \overrightarrow{AC} \times (u \overrightarrow{AC} + v \overrightarrow{AB}) \right| = \frac{1}{2} \left| v \right| \left| \overrightarrow{AC} \times \overrightarrow{AB} \right|$

$S_{\Delta BPC} = S_{\Delta ABC} - S_{\Delta APB} - S_{\Delta APC} = \left(1 - \left| u \right| - \left| v \right|\right) \frac{1}{2} \left| \overrightarrow{AC} \times \overrightarrow{AB} \right|$

可以得到 $\begin{cases} \frac{S_{\Delta BPC}}{S_{\Delta ABC}} = 1 - \left| u \right| - \left| v \right| \\ \frac{S_{\Delta APB}}{S_{\Delta ABC}} = \left| u \right|  \\ \frac{S_{\Delta APC}}{S_{\Delta ABC}} = \left| v \right| \end{cases}$

由于上面的推导是基于 $P$ 在三角形内的前提, 所以 $u$ 和 $v$ 都大于 0, 且 $u + v \le 1$. 可以发现与线性组合的定义是一样的.
#+end_quote

因此, 现在的目标变成了解方程 $\begin{cases}  \overrightarrow{AP} \cdot \overrightarrow{AC} = (u\overrightarrow{AC} + v\overrightarrow{AB}) \cdot \overrightarrow{AC} = u \overrightarrow{AC} \cdot \overrightarrow{AC} + v \overrightarrow{AB} \cdot \overrightarrow{AC} \\ \overrightarrow{AP} \cdot \overrightarrow{AB} = (u\overrightarrow{AC} + v\overrightarrow{AB}) \cdot \overrightarrow{AB} = u \overrightarrow{AC} \cdot \overrightarrow{AB} + v \overrightarrow{AB} \cdot \overrightarrow{AB}\end{cases}$,

#+begin_quote
由于 $\overrightarrow{AB}$ 和 $\overrightarrow{AC}$ 并不一定正交, 所以 $\begin{cases} \overrightarrow{AP} \cdot \overrightarrow{AB} = v \\ \overrightarrow{AP} \cdot \overrightarrow{AC} = u \end{cases}$ *不一定* 成立.
#+end_quote

为了方便阅读, 令 $\begin{cases} \mathbf{a} = \overrightarrow{AC} \cdot \overrightarrow{AC} \\ \mathbf{b} = \overrightarrow{AB} \cdot \overrightarrow{AC} = \overrightarrow{AC} \cdot \overrightarrow{AB} \\ \mathbf{c} = \overrightarrow{AB} \cdot \overrightarrow{AB} \\ \mathbf{d} = \overrightarrow{AP} \cdot \overrightarrow{AC} \\ \mathbf{e} = \overrightarrow{AP} \cdot \overrightarrow{AB} \end{cases} \rightarrow \begin{cases} \mathbf{d} = \mathbf{a}u + \mathbf{b}v \\ \mathbf{e} = \mathbf{b}u + \mathbf{c}v \end{cases}$.

在计算时还需要注意三角形的退化情况, 即三个点共线无法构成具有二维区域的平面图形, 而是退化成为一条线段, 三角形的面值为 0.

方法也很简单, 矩阵的行列式表示变换所带来面积/体积的变化, 而坐标系可以用矩阵表示, 因此可以通过行列式进行判断三角形是否退化:

$\mathbf{D} = \left| \begin{array}{c} \mathbf{a} & \mathbf{b} \\ \mathbf{b} & \mathbf{c} \end{array} \right| = \mathbf{a}\mathbf{c} - \mathbf{b}^2$

如果 $\mathbf{D} \ne 0$, 那么三角形就是非退化的情况, 可以通过可莱姆法则来解出 $\begin{cases} u = \frac{\mathbf{d}\mathbf{c} - \mathbf{b}\mathbf{e}}{\mathbf{D}} \\ v = \frac{\mathbf{a}\mathbf{e} - \mathbf{b}\mathbf{d}}{\mathbf{D}} \end{cases}$;

如果 $\mathbf{D} = 0$, 那么三角形就是退化的情况, 需要把问题转化成求点 $Q$ 与线段距离的问题.

# $\Delta = $

# https://blog.csdn.net/chaser30/article/details/141993529

# https://chat.deepseek.com/a/chat/s/d5b99b2a-5ec1-44d2-983e-5518ae55d6be

*方法二*

构建出一个由 $\overrightarrow{AC}$, $\overrightarrow{AB}$ 和 $\overrightarrow{AC} \times \overrightarrow{AB}$ 组成的参考系,

$\overrightarrow{AQ} = u \overrightarrow{AC} + v \overrightarrow{AB} + w \left(\overrightarrow{AC} \times \overrightarrow{AB}\right)$
...
通过计算相邻片元的深度差, 并找出其中最大的深度差.

#+begin_quote
=3D Game Shaders For Beginners= 所用坐标系的 $z$ 轴是向上,  $y$ 轴是指向屏幕内,

与我们使用视点坐标的 $z$ 分量作为线性深度不同, 它的视点坐标的 $y$ 分量才是片元的线性深度.
#+end_quote

#+caption: 图片来源于 3D Game Shaders For Beginners
[[../../../files/FRBkSm1.png]]

如果存在较大深度差, 就说明该当前片元所对应的顶点处于物体的轮廓上.

这种方法很简单易懂, 轮廓效果取决于深度差的阈值设定, 这里就不详细介绍了.

接下来会介绍第二种方法: [[../../2024/03/code-explains-for-fragment-shader-in-shadertoy.html#sobel][Sobel核]].

在图像处理中, =Sobel核= 是用于图像边缘检查的, 原理是计算当前片元在各个方向的导数并进行统计,

这些导数称为方向导数, 能够很好地反应出局部范围内的片元变化规律.

=Sobel核= 的统计结果能够反映出当前片元与边缘的相似程度, 相似程度越高, 统计结果越大.

由于灰阶(gray scale)图更能突显图像的物体的轮廓, 因此为了提高连续性判断的准确性,
...
看到阴影区域这个词, 通常第一反应是阴影贴图, 然而阴影贴图只能找到 *直射光线* 被遮蔽的区域.

物体的亮度是由直射光线和 *间接光线* 一同贡献的, 这就是为什么能够观察到那些没受到直射光线照射的区域.

因此, 本章节范围内重新定义阴影区域: 低亮度的区域, 或者说受到较少光照的区域.

为了直观理解低亮度与直射光线被遮蔽之间的区别, 可以看以下图片.

#+caption: 原场景 (左), 原场景中光线被遮蔽的区域 (右)
[[../../../files/luma-vs-shadow.png]]

根据图像, 物体较暗的地方并不是直射光线被遮蔽造成的, 而是光照计算结果偏低的结果.

也可以把物体的阴影选项关闭进行对比, 可以发现去掉阴影后, 物体的低亮度区域依然存在.

#+attr_html: :width 600px
#+caption: 关闭物体的阴影选项
[[../../../files/scene-without-shadow.png]]

那么该如何找出阴影区域呢? 思路如下:

首先要使用亮度函数把场景渲染结果转换成亮度灰阶图:
...
当阴影区域的亮度达到最大等级, 在阴影区域上绘制斜线作为阴影线;

如果亮度达到中级, 在原有阴影线上 *添加* 垂直线作为阴影线;

如果亮度达到最小等级, 在原有阴影线 *添加* 水平线作为阴影线;
...
这里的亮度并非光照计算的结果, 而是基于场景色彩计算的筛选结果,

有些场景区域的表面本来就深色, 哪怕是处于直射光线之下也有可能被判定为阴影区域.
...
针对场景颜色过深导致被误判为阴影区域的问题, 本人有一种未经实践的想法:

计算出光照计算中的 *漫反射系数* 以及 *镜面反射系数* 之和 $S \ge 0$ 作为亮度, 基于此亮度判断是否处于阴影区域.

但 $S$ 并没有一个固定的范围, 不好划分阴影等级, 或许可以使用 =Reinhard= 映射来把 $S$ 映射到 $[0, 1)$ 中,
...
# 其实通过阴影贴图来判断片元是否处于阴影区才是正确的做法.

# 但计算阴影的话题内容最少需要写一个章节, 再多就是写一篇文章,

# 如果有兴趣可以阅读:

# [[../../2020/08/graphics-opengl-light-and-material.html#shadow-mapping][图形学 - 光和材质: 阴影]]

# [[../../2022/02/webgl-buffer-objects.html#fbo-in-threejs][Shader 编程自救指南: Three.js 中使用帧缓冲]] (这里面有介绍如何使用 =three.js= 内置帧缓冲, 包括阴影贴图)
# #+end_quote
...
第一, 考虑光照计算结果是否参与进行素的亮度计算, 如果参与该如何计算; 第二, 只有非阴影区域才有能高亮反射区域.
...
可以发现高亮反射区域的描边在刚进入阴影区时没有完全消失.

#+attr_html: :width 600px
#+caption: 阴影区出现了高亮反射区的轮廓
[[../../../files/moebius-normal-drawback.png]]

解决办法是根据场景亮度在标记高亮区域, 只有不在阴影区才能有机会标记为高亮.
...
#+begin_src glsl
  ...
  void main () {
    vec4 modelPosition = modelMatrix * vec4(position, 1.0);
    vec4 clip = projectionMatrix * viewMatrix * modelPosition;
    gl_Position = clip;

    vNormal = normalize(normalMatrix * normal);
    vPosition = modelPosition.xyz;
    // 计算出顶点的相机 UV, 因为是要在场景贴图上采样,
    // 生成场景贴图的相机正好是当前成像用的相机
...
#+begin_src glsl
  ...
  void main () {
    ...
    vec4 sceneColor = texture2D(tDiffuse, vScreenUV);
    // 进行亮度计算, 需要与前面的亮度计算进行统一
    float pixelLuma = clamp(luma(sceneColor.xyz) + kDiffuse * 0.17, 0., 1.);

    vec3 color = vec3(vNormal * 0.5 + 0.5);

    // 当亮度小于 LOW_LUMA_1 (假设为 0.32) 就认为进入阴影区域, 这里去掉了使用 kDiffuse 进行判断
    if (pixelLuma > 0.32 && kSpecular >= .5) {
      color = vec3(0.0);
    }
...
#+attr_html: :width 600px
#+caption: 修复高亮区域出现在阴影区的问题: 高亮区域逐渐消失在阴影边缘附近
[[../../../files/moebius-normal-drawback-fixed.png]]
...
*** 风格化渲染: 卡通着色 (Cel Shading / Toon Shading / Carton Shading)
...
$L_{d} = \frac{L_{w}}{1 + L_{w}} \rightarrow L_{w} = \frac{L_{d}}{1 - L_{d}}$
