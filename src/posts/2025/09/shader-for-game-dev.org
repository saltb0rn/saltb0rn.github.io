...
关键在于动态的场景需要事实计算立方体贴图, =three.js= 的 [[https://threejs.org/docs/?q=CubeCamera#api/en/cameras/CubeCamera][CubeCamera]] 就是用于把场景写入到立体贴图中.
...
#+BEGIN_SRC glsl
  ...
  void main() {
    ...
    vec4 projPos = projectionMatrix * viewMatrix * vWorldPos;
    vDepth = projPos.z;
    gl_Position = projPos;
  }
#+END_SRC
...
*** COMMENT 屏幕空间反射 (Screen Space Reflection, SSR)
*** 屏幕空间环境光遮蔽 (Screen Space Ambient Occlusion, SSAO)
:PROPERTIES:
:CUSTOM_ID: ssao
:END:

屏幕空间环境光遮蔽是由 =Crytek= 开发的渲染技术, =2007= 年首次应用在游戏  =Cyrsis= 中,

用于估算场景中物体表面因遮挡形成的阴影区域, 计算过程是发生在屏幕空间(或称屏幕坐标系)中的,

通俗点就是后处理:

1. 把场景部分几何信息(顶点坐标, 法线, 颜色, 深度等)写入贴图中,
   这些贴图被称为几何缓冲(=geometry buffer=) 或 =G-Buffer=;
2. 在全屏渲染着色器(=shader=)中按照 =UV= 坐标从 =G-Buffer= 中读取信息进行计算;
3. 把计算结果输出到屏幕上, 或者写入到贴图中给另外一个着色器使用.
...
根据屏幕坐标从顶点贴图 =tViewPosition= 获取实际成像的顶点坐标, 该顶点坐标的 $z$ 分量就是实际成像的深度值 =depth=.
...
其中 $z$ 是 $s$ 的 $z$ 分量, $\mathrm{depth}$ 是根据 $s$ 在 =tViewPosition= 上获得的深度值, $r$ 是半球体的半径.
...
#+BEGIN_SRC glsl
  ...
  void main() {
    ...
    vec3 origin = texture2D(tViewPosition, uv).xyz;
    vec3 normal = (texture2D(tViewNormal, uv) * 2.0 - 1.0).xyz;
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
      ...
      vec3 surfaceView = origin.xyz + dir * radius;
       // Clip Space
      vec4 surfaceClip = projectionMatrix * vec4(surfaceView, 1.0);
       // NDC
      vec3 surfaceNDC = surfaceClip.xyz / surfaceClip.w;
       // Screen Space
      vec2 surfaceUV = (surfaceNDC.xy * 0.5 + 0.5).xy;

      vec4 sampleDepth = texture2D(tViewPosition, surfaceUV).z;
...
#+BEGIN_SRC glsl
  ...
  void main() {
    ...
    for (int row = -1; row <= 1; row++) {
      float y = float(row);
      vec2 left = vec2(-1.0, y) * texelSize;
      vec2 mid = vec2(0.0, y) * texelSize;
      vec2 right = vec2(1.0, y) * texelSize;

      result += texture2D(tSSAO, left).r;
      result += texture2D(tSSAO, mid).r;
      result += texture2D(tSSAO, right).r;
    }

    gl_FragColor = result / (3.0 * 3.0);
  }
#+END_SRC

*** COMMENT 延迟渲染 (Deferred Rendering)
...
https://lettier.github.io/3d-game-shaders-for-beginners/deferred-rendering.html

https://baijiahao.baidu.com/s?id=1804272498713334704&wfr=spider&for=pc
