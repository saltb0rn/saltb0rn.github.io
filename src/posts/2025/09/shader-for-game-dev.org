...
#+caption: 图片来源于[[https://zhuanlan.zhihu.com/p/1912925340386441129][知乎]]
...
函数 $f$ 是一个倒数函数 (=reciprocal function=), 每次移动一个像素的距离时, $(B_{view})_z$ 的变化是线性的,

但深度变化 $(r_{uvw})_{z}$ 不是, 因此关系 $d_{w} = \frac{|d_{frag}|}{|r_{screen}|} \times (r_{uvw})_{z} = \frac{(r_{uvw})_{z}}{maxComp}$ *不成立*.

因此在透视投影中, 只能在移动像素时想办法拿到像素所对应视点坐标的 $z$ 分量, 从而计算出深度. 这挺简单的,

假设有一个片元坐标 $P_{screen} = A_{screen} + i \times d_{frag}, i \in [0, maxComp]$,

$P_{screen}$ 为从 $A_{view}$ 向 $B_{view}$ 移动时某个中间点 $P_{view} = (1 - t) \times A_{view} + t \times B_{view}, t = \frac{i}{maxComp} \in [0, 1]$ 在屏幕空间上的片元坐标,

(视点坐标的变化也是均匀的,) 一旦拿到了 $P_{view}$ 就能很容易根据 $(P_{view})_{z}$ 计算出 $P_{screen}$ 对应的非线性深度:
...
数学上只要是可以线性插值的情况, 那必然可以外推, 因此 $P_{view}$ 的计算在 $t \in [0, 1]$ 之外的情况也成立.
...
1. 把当前相机看到的景象保存为颜色贴图 =tDiffuse=, 用于反射采样
2. 找出 $O$ 点上的反射光 $R_0$ 与场景的交点 $P$
3. 计算出交点 $P$ 在相机中的 =UV= 坐标, 并根据该 =UV= 坐标从 =tDiffuse= 获取反射信息
4. 利用反射信息对 $O$ 点进行着色, 从而实现反射


其中第 2 步找反射光 $R_0$ 和场景的交点是屏幕空间反射实现的重点与难点,

可以不夸张地说, 如果能够发展出一种算法复杂度为 $O(1)$ 的求交点算法,

图形学一大堆技术的性能问题都可以被解决, 比如实时光追.

求交点的技术在计算焦散中的 [[#find-hit-point-with-raymarching][在光栅化渲染中使用 Ray Marching 找出射线与场景的交点]] 一节中有介绍逐个像素移动求交点的方法,

但这种方法的效率不太好, 用于求交点的深度贴图分辨率越高, 找到交点所需的循环次数越多.

后面在实现屏幕空间反射时会在这个方法的基础上做出一些调整以提高效率.

整个屏幕空间反射的实现需要以下贴图:

1. 求射线与场景的交点需要用到深度贴图 =tDepth=;

2. 求反射光线要用到视点空间上的顶点贴图 =tViewPosition= 和法线贴图 =tViewNormal=;

3. 场景采样 =tDiffuse=;

4. 用于标识片元是否支持反射的贴图 =tMirror=.


其中 [[#view-pos-buf][tViewPosition]] 和 [[#view-normal-buf][tViewNormal]] 的生成方法和屏幕环境遮蔽中用到的同名贴图是一致的, 就不赘述了;

由于深度贴图是用来查找交点, 因此可以把片元深度顺便储存到 =tViewPosition= 的 =w= 通道上;

片元反射的标识也只需要一个通道, 可以把 =tViewNormal= 的 =w= 通道用上, 完全没必要额外生成 =tMirror=.

**** 调整后的视点空间贴图生成程序

[[app-vertex-position-view][*Vertex Shader*]]
...
#+BEGIN_SRC glsl
  ...
  void main() {
    // gl_FragCoord.z 的范围是 [0, 1]
    gl_FragColor = vec4(vPosInViewSpace.xyz, gl_FragCoord.z);
  }
#+END_SRC
...
屏幕空间虽然有性能优势, 但屏幕空间反射只能反射当前相机可见的场景内容, 反射光线与场景的交点出现在相机视野外时无法被反射.
