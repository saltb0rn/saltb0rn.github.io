...
在逻辑中, 如果 $A$ 成立导致 $B$ 成立, 但不排除其它情况导致 $B$ 成立, 那么 $A$ 就是 $B$ 的充分条件,

换而言之, $A$ 成立一定会导致 $B$ 成立, 当 $B$ 成立不一定是 $A$ 导致的;

如果 $A$ 要与额外条件一同成立才导致 $B$ 成立, 那么 $A$ 就是 $B$ 的必要条件,

换而言之, 只有 $A$ 成立时 $B$ 才 *有可能* 成立, 如果 $A$ 不成立那么 $B$ 一定不成立;

如果 $A$ 成立那 $B$ 必定成立, 且不存在其它情况使 $B$ 成立, 那么 $A$ 就是 $B$ 的充分必要条件.
