...
解决这个问题除了把注释解除掉外, 还可以在使用 =GCC= 编译时让它链接到一个提供定义的二进制文件,

这种提供定义的二进制文件就是人们口中的库.
...
#+BEGIN_SRC sh
  gcc -o simple-c simple-c.c simple-c-func.c -static
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  gcc -c simple-c.c
  gcc -o simple-c simple-c.o simple-c-func.o -static
#+END_SRC

=-static= 是 =GCC= 告诉其调用的链接器把外部引用全部打包编译结果中.

举一个简单的例子, 如果 =simple-c-func.c= 有引用到 =libc= 里面的 =printf= 等函数,
...
那么 =printf= 会从 =libc= 中被提取出来, 成为不包含任何外部引用的独立存在, 这是一个递归的过程,

这里以一个 *假设* 的过程来表面思路:

  1. =main= 只调用了 =simple-c-func.c= 的 =func()=

  2. =func()= 只调用了 =libc.a= 的 =printf()=

  3. =printf()= 由 =libc.a= 中的 =printf.o= 提供

  4. =printf()= 只调用了 =libc.a= 的 =write()=

  5. =write()= 由 =libc.a= 中 =write.o= 提供

  6. =write()= 没有任何外部引用

  7. 把检索到的模块链接在一起 =ld -o simple-c.o simple-c-func.o print.o write.o=

*静态链接: 创建静态库, 与静态库链接*

#+BEGIN_SRC sh
  gcc -c -o simple-c-func.o simple-c-fun.c
  # 创建静态库: 使用归档命令 ar 把模块文件打包
  ar rcs libsimple-c-func.a simple-c-func.o
  gcc -o simple-c simple-c.c -lsimple-c-func -L. -static

  # 为了程序安全, 可把 simple-c 编译成静态位置无关可执行程序(PIE)
  gcc -static-pie -fPIE -pie -o simple-c simple-c.c -lsimple-c-func -L. -static
#+END_SRC

这里的 =-lsimple-c-func= 就是让 =simple-c= 和 =simple-c-func.a= 进行链接,

=GCC= 的 =-l= 选项就是用来指定要链接的链接库名;

=-L= 指定在哪个路径下找链接库, 如果库在系统默认位置上, 那么不需要该选项指定.
...
#+BEGIN_SRC sh
  gcc -o simple-c simple-c.c simple-c-func.c -dynamic
  # 或
...
#+BEGIN_SRC sh
  ...
  # 或为了保险起见, 生成位置无关代码(PIC)的动态库, 上面的动态库有可能无法正常工作
  gcc -shared -fPIC simple-c-func.c -o libsimple-c-func.so

  # 也可以这么做
  gcc -fPIC simple-c-func.c -o simple-c-func.o
  gcc -shared simple-c-func.o -o libsimple-c-func.so

  # 执行链接
  gcc -o simple-c simple-c.c -lsimple-c-func -L. -Xlinker -rpath -Xlinker .
  # 为了程序安全, 可把 simple-c 编译成可动态链接的位置无关可执行程序(PIE),
  # 这种程序每次执行的内存地址都不一样
  gcc -fPIE -pie -o simple-c simple-c.c -lsimple-c-func -L. -Xlinker -rpath -Xlinker .
#+END_SRC

=-shared= 告诉 =GCC= 把代码编译成动态链接库,

需要注意这里用上了 =GCC= 的 =-Xlinker= 选项来给 =ld= 传入 =-rpath .=,

由于编译得到的程序的文件格式是 =ELF=, 并且我们这个库并非在系统搜索的路径里面,

因此我们需要告诉 =ld= 该链接生成的程序在 *运行时* 需要从 =-rpath= 给定的位置 =.= 上查找 =libsimple-c-func.so=,
...
这里有一个地方值得思考一下, 如果 =ld= 默认的库查找路径上同时存在一个库的动静态两个版本的库, =ld= 又是如何选择的呢?
...
你没办法在系统路径上找到 =libstdio.a= 或者 =libstdio.so= 的文件,

你可以 =ldd= 命令看一个可执行程序或者动态链接库链接了哪些动态链接库,
...
但这种方法不方便对库进行管理, 也增加手动操作的情况.

后来有人想到了把系统轻量化后打包到容器中, 把容器当作程序运行在真正的系统上,

这么一来, 容器与真正的系统就形成环境隔离. 开发的程序只要能在容器上进行开发和运行,

部署时就无需担心真系统的环境是否满足程序要求. 容器管理的技术有很多, 比如 =Docker= 公司的 =docker=,

=docker= 早期所使用的容器技术是 =2008= 年的 =Linux Containers (LXC)=.

传统虚拟机技术, 是通过模拟出完整的虚拟硬件(CPU/内存/网卡,等等)为某个操作系统提供完整的运行环境,

要运行多少个操作系统, 就需要模拟出多少个运行环境, 运行环境之间的资源是相互隔离的;

=LXC= 是通过 =Linux= 内核提供的 =Namespace= 和控制组(=cgroups=)实现进程与资源的隔离, 从而形成独立的运行环境,

所有的这些运行环境都是在同一个 =Linux= 内核上.

相比于传统虚拟机技术, =LXC= 的启动更快, 资源占用更少, 但配置复杂, 用户体验不佳,

起初 =docker= 是为了降低 =LXC= 的使用门槛, 后来 =Docker= 公司去掉了对 =LXC= 的依赖,

自己实现了 =libcontainer= (后来改名成 =runc=)来实现虚拟化, 其原理与 =LXC= 的一样,

并在 =2015= 年 =6= 月 =22= 日牵头其他商业公司共同成立了基金会,

定制了一套 *容器运行时* 和 *镜像格式* 的标准规范: =Open Container Initiative (OCI)=,

把 =libcontainer= 捐赠出去成为标准的一部分, 但这一波技术开源成为 =Docker= 公司在与大公司的商业竞争中落败的原因.

有意思的是, 作为 =docker= 创建容器的配置文件 =Dockerfile= 虽然没成为标准的一部分, 但依然被 =docker= 的竞品所使用.

因此, 开发者想要学习容器的使用, 主要去学 =Dockerfile= 的编写即可.

目前为此, =C= 语言不像 =Rust/JavaScript= 那样每个项目都有自己的的依赖管理, =docker= 在一定程度上解决了这个问题.
...
=C/C++= 语言的声明语法基本上都是一样, 并且都很难阅读, 个人觉得直接看它们的声明语法也不太直观,

你可以在[[https://www.open-std.org/jtc1/sc22/wg14/www/projects#9899][这里]]找到 C 语言的标准, 看看里面的 =Language-Declarations= 那一节.
...
之所以可行, 是因为在固定架构下, =CPU= 寻址大小是固定的, 所谓内存寻址是指 =CPU= 允许支持的内存大小,

内存地址本身的大小是固定的, 既然指针变量储存的是内存地址, 那么所有类型的指针变量的大小都是一致的.

而指针变量的类型则是告诉编译器: 指针指向的内存区块有多大. 内存区块的大小只有在解引时才有意义,

因为需要知道应该读多少个字节的连续内存, 而不是把区块以外的数据也读取了.

我们可以把物理内存看作是一个数组, 内存地址就是数组元素的索引, 因此内存地址就是一个整数.

=C= 语言的标准定义了一种专门储存内存地址的非指针类型 =uintptr_t= 以及 =intptr_t=,

不用 =int= 来储存则是因为 =int= 的大小不适合用来储存内存地址.
