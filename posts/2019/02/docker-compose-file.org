...
**** docker-compose.yml 配置
     ...
     关于如何确保 =mysql= 服务在 =music.api= 服务启动之前就绪,我在网上看到几种说法, =wait-for-it= 和 =wait-for= 等脚本,也有人说 =healthcheck=,但是我看了文档并没觉得这选项能干嘛(也有可能根据它的检测结果来决定是否启动 =music.api= 服务?).
...
**** 关于 MYSQL Docker 的额外补充
     ...
     *要注意的是* 当 =/var/lib/mysql= 已经有数据的时候, =init.sql= 就不会在下一次服务启动的时候被执行了.
     ...
     还有就是 =MYSQL_USER=, =MYSQL_ROOT_PASSWORD=, =MYSQL_DATABASE= 等等这些变量,分别是说启动镜像的时候创建出用户,设定 =root= 密码和创建数据库,也就是相当于上面 =init.sql= 文件的内容,在 =init.sql= 准备好的情况下,这些变量都是可选的.
