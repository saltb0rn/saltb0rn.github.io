...
上面的 =interpreters= 都使用到 =environment= 计算, =environment= 记录了上下文的数据绑定,是数据上下文的一种抽象(an abstraction of data context).

除了记录上下文的数据外,还可以记录还有多少没计算完/下一步的执行起始点,这里引入 =continuation= 的概念,是控制上下文的一种抽象(an abstraction of control context).

如果还不能理解什么是控制上下文,可以看以下这个例子,
...
当我们要理解 =(execute 5)= 的执行过程,要看函数定义,然后按照执行顺序一步一步读,这个例子整个执行顺序为
